...
def _wanted(relative: str) -> bool:
    return relative.startswith(_DATA_PREFIXES)
...
def _directory_records(
    ...
    *,
    ...
    if variant == IMPROVEMENT2_VARIANT and source_projection:
        compat = package_dir / "compat" / IMPROVEMENT2_CONSUMER / "improvement"
        if not compat.is_dir():
            raise ProjectCommandError(
                f"improvement2 compatibility projection is missing: {compat}"
            )
        for path in sorted(compat.rglob("*")):
            if path.is_file():
                yield f"improvement/{path.relative_to(compat).as_posix()}", path.read_bytes()
        roots = ("equipment", "assets/equip", "assets/useitem")
    else:
        roots = ("improvement", "equipment", "assets/equip", "assets/useitem")
...
def _safe_member_relative(name: str) -> str | None:
    ...
    return relative if _wanted(relative) else None
...
def inspect_tarball(tarball: Path, *, variant: str) -> dict:
    return _digest_records(variant, _tarball_records(tarball))
