...
The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. `data-build.yml` freezes both tarballs, versions, hashes, and required dist-tags in one immutable Candidate Bundle; `release.yml` verifies that bundle, publishes or reconciles `latest` and `improvement2`, and then updates the `online` generated-state branch. Public main release is independent and does not publish npm data.
