...
对应 GitHub Actions 为 `source-acquire.yml`、`data-build.yml` 和 `release.yml`。Data Build 在分配版本前直接读取当前数据目录与 npm Registry tarball 的实际消费者文件，并按规范化规则计算两类身份：canonical schema-4 digest 与 `improvement2` schema-3 投影 digest。版本号、生成时间、CHANGELOG、RELEASES 和审计元数据不参与身份。canonical digest 不同才分配新 patch；相同则复用 npm 版本。若同版本 `improvement2` 已存在，其实际投影 digest 必须与当前投影一致，否则作为不可变 Registry 冲突停止。缺失变体、dist-tag 与 `online` 自动对账；`online` 仅是 npm 正式状态的投影。原始网页缓存、浏览器会话和运行日志不进入公开分支。
