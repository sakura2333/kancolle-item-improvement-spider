...
公开自动化拆分为三个职责：`source-acquire.yml` 只采集并冻结 Source Bundle；`data-build.yml` 消费冻结来源，完成离线计算、验证和不可变 Candidate，并在真实消费者数据变化时自动发布 `latest`、`improvement2` 和 `online`；`release.yml` 只用于从既有冻结 Candidate 补偿或对账发布。版本规划不再信任包内自报的历史 digest，而是直接从当前目录和 npm Registry tarball 的实际消费者文件重算身份：canonical schema-4 与 `improvement2` schema-3 投影分别拥有稳定 digest。`generatedAt`、`packageVersion`、CHANGELOG、RELEASES 和审计元数据均不参与数据身份。canonical digest 相同永远复用 npm 已有版本；已存在的 `improvement2` 必须与当前兼容投影 digest 一致，否则视为 npm 不可变版本冲突。缺失变体、dist-tag 或 `online` 由本次 Build 自动对账，不依赖上一次 Workflow 或原 Candidate，也不会再次分配 patch。`online` 只是 npm 正式状态的可修复投影。
