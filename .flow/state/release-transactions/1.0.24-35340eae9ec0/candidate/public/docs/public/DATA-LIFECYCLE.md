...
Data Build 先完成离线计算和质量检测，再读取 npm 最高正式版本的 `RELEASES.json` 并比较消费者 `contentDigest`，之后才允许版本规划和不可变 Candidate 上传。digest 不同才分配新 patch；digest 相同则把 npm 已有版本视为唯一目标；canonical 与已存在的 `improvement2` 必须各自证明携带同一消费者 digest，冲突时停止且不得推进 dist-tag 或 generated-state。验证通过后再自动补齐缺失的 canonical/`improvement2` dist-tag 与 generated-state。已有 npm tarball 会直接下载并冻结进本次 Candidate，缺失变体才从当前同 digest 数据生成；`online` 缺失或落后不会触发升版，只会触发同版本对账。全部状态一致时以成功 no-op 结束。
