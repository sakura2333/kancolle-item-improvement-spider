...
对应 GitHub Actions 为 `source-acquire.yml`、`data-build.yml` 和 `release.yml`。Data Build 在分配版本前读取 npm 最高正式版本中的消费者 `contentDigest`。digest 不同才分配新 patch；digest 相同则直接复用 npm 版本；已存在的 canonical 与 `improvement2` tarball 必须分别验证其 `RELEASES.json.contentDigest` 且两者一致，随后才允许从当前冻结结果自动补齐缺失的 `improvement2`、dist-tag 与 `online`。该判断不依赖上一次 Workflow、审计文件或 `online` 是否及时推进；`online` 仅是 npm 正式状态的投影。原始网页缓存、浏览器会话和运行日志不进入公开分支。
