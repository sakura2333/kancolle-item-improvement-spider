...
公开自动化拆分为三个职责：`source-acquire.yml` 只采集并冻结 Source Bundle；`data-build.yml` 消费冻结来源，完成离线计算、验证和不可变 Candidate，并在消费数据变化时自动发布 `latest`、`improvement2` 和 `online`；`release.yml` 只用于从既有冻结 Candidate 补偿或对账发布。版本规划前会读取 npm 最高正式版本的 `RELEASES.json`，用消费者 `contentDigest` 判定该数据是否已发布。相同 digest 永远复用 npm 已有版本：现有 canonical 与 `improvement2` tarball 都必须从各自的 `RELEASES.json` 证明绑定同一个消费者 digest；不一致视为 npm 不可变版本冲突并停止。缺失的 `improvement2`、dist-tag 或 `online` 会由本次 Build 自动对账，不依赖上一次 Workflow 或原 Candidate，也不会再次分配 patch。`online` 只是 npm 正式状态的可修复投影，不再作为升版事实源。下载与计算仍不共享执行阶段，发布只消费已经验证并上传的 Candidate。
