...
对应 GitHub Actions 为 `source-acquire.yml`、`data-build.yml` 和 `release.yml`。Data Build 在分配版本前读取 npm 最高正式版本中的消费者 `contentDigest`；只有 digest 尚未发布且发布计划为真时，才在冻结、验证并上传 Candidate 后自动发布 npm 并更新 `online`。若相同 digest 已发布但 `online` 未对齐，流程停止并要求 Release 工作流使用原 Candidate 补偿，禁止继续递增版本。原始网页缓存、浏览器会话和运行日志不进入公开分支。
