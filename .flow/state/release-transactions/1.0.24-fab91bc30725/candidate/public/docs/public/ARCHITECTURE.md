...
对应 GitHub Actions 为 `source-acquire.yml`、`data-build.yml` 和 `release.yml`。Data Build 在冻结、验证并上传 Candidate 后，只有发布计划为真时才自动发布 npm 并更新 `online`；Release 工作流只用于同一 Candidate 的补偿与对账。原始网页缓存、浏览器会话和运行日志不进入公开分支。
