...
Data Build 先完成离线计算、质量检测、版本规划和不可变 Candidate 上传。消费数据未变化时以成功 no-op 结束；数据变化时，独立 Publish Job 才获得 npm 凭据与 `online` 写权限，并只发布 Candidate 中已经冻结和复验的 `latest`、`improvement2` 与 generated-state。手动 Release 只用于同一 Candidate 的失败补偿与对账。
