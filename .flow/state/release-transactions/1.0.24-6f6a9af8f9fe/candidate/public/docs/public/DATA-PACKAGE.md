...
来源采集输出 Source Bundle；计算工作流从该 Bundle 生成不可变 Candidate。版本规划使用消费者数据的 `contentDigest`，并在分配新 patch 前读取 npm 最高正式版本的 `RELEASES.json`：digest 相同表示该数据已经发布，只允许补偿 `online`，不得重新打包成新版本；digest 不同才会验证、上传并自动发布 Candidate 中冻结的 npm tarball 和 online-state。手动发布工作流只用于补偿或对账；相同 Candidate 可以在不重新抓取、不重新计算的情况下重试发布。
