...
Data Build 先完成离线计算和质量检测，再读取 npm 最高正式版本的 `RELEASES.json` 并比较消费者 `contentDigest`，之后才允许版本规划和不可变 Candidate 上传。消费数据未变化时以成功 no-op 结束；digest 尚未发布时，独立 Publish Job 才获得 npm 凭据与 `online` 写权限，并只发布 Candidate 中已经冻结和复验的 `latest`、`improvement2` 与 generated-state。若相同 digest 已在 npm 存在但 `online` 未对齐，Build 必须停止，手动 Release 使用原 Candidate 补偿，不能生成下一个 patch。
