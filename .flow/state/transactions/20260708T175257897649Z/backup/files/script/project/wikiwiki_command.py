...
def _parse(args: list[str]) -> tuple[str, bool, bool, int | None, bool] | dict:
    ...
    if action != "probe" and skip_start2:
        return _usage_error("--skip-start2 只供 ./flow smoke 内部调用")
...
def run(root: Path, args: list[str], config: dict, loader=None) -> dict:
    ...
    if receipt and not receipt.get("ready"):
        ...
        if action == "probe":
            return result(
                ...
                completed,
                incomplete,
                "完整小样本链路请执行 ./flow smoke；full source acquisition 请执行 ./flow wikiwiki --full",
                "无需回滚；source cache 与断点保留在 .flow/local",
            )
        return result(
            "成功",
    ...
    if action == "probe":
        ...
        next_step = "完整小样本链路请执行 ./flow smoke；full source acquisition 请执行 ./flow wikiwiki --full"
    elif full:
