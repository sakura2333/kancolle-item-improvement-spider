...
- `api_mst_useitem` 提供消耗品 ID；Spider 按官方 `useitem/card` 与旧式 `card_` 路径获取 PNG，编码缓存位于 `.spider/local/source-cache/cache/useitem/webp-q93-a100-m6-exact/{id}.webp`。`latest` 发布 `assets/useitem/{id}.webp`，`improvement2` 仅发布 `assets/useitem/{id}.png`。
