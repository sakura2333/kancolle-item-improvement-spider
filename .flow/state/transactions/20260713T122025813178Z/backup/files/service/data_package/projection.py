...
def _promote_cached_icons():
    """Persist canonical WebP and legacy PNG use-item assets.

    The official PNG remains available for the improvement2 compatibility
    projection, while the canonical package consumes the deterministic Q93
    WebP generated from the same source image.
    """
...
def _promote_cached_equipment_images():
    """Persist official equipment WebP cards into a stable asset directory."""

    STATIC_EQUIPMENT_IMAGE_DIR.mkdir(parents=True, exist_ok=True)
    for legacy_png in STATIC_EQUIPMENT_IMAGE_DIR.glob("*.png"):
        legacy_png.unlink()
