...
class CacheFreshnessTest(unittest.TestCase):
    ...
    def test_strict_download_pic_reuses_cache_within_half_year_without_network(self):
        ...
        meta = {
            cache.cache_key(str(path)): {
                "url": url,
                "fetched_at": int(time.time()) - (179 * 24 * 3600),
                "etag": '"fixture-image"',
                "fetch_status": "fresh",
                "used_cache_fallback": False,
            }
    ...
    def test_download_pic_uses_half_year_cache_ttl(self):
