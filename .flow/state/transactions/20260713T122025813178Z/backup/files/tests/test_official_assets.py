...
class OfficialAssetTest(unittest.TestCase):
    ...
    def test_invalid_cached_webp_is_reencoded(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            target.touch()
            actual = official_assets._encode_webp(
                source,
                target,
    ...
    def test_useitem_card_keeps_png_and_generates_canonical_webp(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            source = root / "88.png"
            webp_dir = root / "webp"
            ...
            with patch.object(
                ...
                official_assets, "CACHE_USEITEM_WEBP_DIR", webp_dir
            ...
            target = webp_dir / "88.webp"
            with Image.open(target) as image:
                self.assertEqual(image.format, "WEBP")
                self.assertEqual(image.size, (270, 270))
