...
Akashi 路线转换完成后，由独立 official-assets 阶段按最终引用 ID 获取官方 useitem/equip 原图并保存到 Source Cache；图片缓存有效期为 180 天。`latest` 把 useitem 与装备统一发布为 `{id}.webp`，`improvement2` 仅保留旧插件需要的 schema-3 改修数据和 `assets/useitem/{id}.png`。
