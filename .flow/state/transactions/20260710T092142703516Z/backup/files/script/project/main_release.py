...
from script.project.npm_release_set import build_release_set
...
def _review_template(state_root: Path, branch: str, commit: str) -> Path:
    ...
    value = {
        ...
        "reviewedAreas": [
            ...
            "improvement2 release projection boundary",
        ],
        "blockingIssues": [],
        "notes": "Replace this note with the AI review summary. Commit any cleanup before approving.",
    }
...
def prepare(*, confirm: bool) -> dict[str, Any]:
    ...
    if latest.is_file():
        try:
            state_root, manifest = stable_command._load_latest(
                PROJECT_ROOT, config, require_npm=False
            )
        except stable_command.StablePreviewStaleError:
            state_root, manifest = stable_command._preview(
                PROJECT_ROOT, config, freeze_npm=False
            )
    else:
        state_root, manifest = stable_command._preview(PROJECT_ROOT, config, freeze_npm=False)
    ...
    if isinstance(recorded, dict) and recorded.get("name"):
        ...
        template = _review_template(state_root, branch, existing)
        return {
    ...
    template = _review_template(state_root, branch, commit)
    return {
        "status": "prepared",
        "releaseId": manifest["releaseId"],
        "branch": branch,
...
def approve(*, review_report: Path) -> dict[str, Any]:
    config = load_runtime()
    state_root, manifest = stable_command._load_latest(PROJECT_ROOT, config, require_npm=False)
    ...
    with tempfile.TemporaryDirectory(prefix="spider-reviewed-candidate-") as temp_name:
        ...
        try:
            ...
            _run(
                [sys.executable, "-m", "compileall", "-q", "service", "pojo", "util"],
                worktree,
            )
            npm_release = build_release_set(worktree, state_root / "npm", require_fresh=True)
            stable_command._validate_npm_release_policy(config, npm_release)
    ...
    manifest["npmRelease"] = stable_command._npm_release_binding(state_root, npm_release)
    manifest["reviewBranch"].update({
        "approvedCommit": commit,
        "approvedTree": tree,
    })
    manifest["mainPublished"] = False
    manifest["npmPublished"] = False
...
def reconcile(*, mark: bool) -> dict[str, Any]:
    ...
    try:
        if gate.get("releaseId"):
            ...
            state_root, manifest = stable_command._load_latest(PROJECT_ROOT, config, require_source_head=False, require_npm=False)
        facts["preview"] = {
            "releaseId": manifest["releaseId"],
