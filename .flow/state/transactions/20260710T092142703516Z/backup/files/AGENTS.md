...
7. 涉及 Python 构建或验收时，读取 `requirements.txt` 与 `script/project/init_env.py`
...
- 核心目录（`flow`、`script`、`service`、`pojo`、`configs`、`packages`）不得 import 或调用 `tools/**`。工具只允许单向读取导出数据，或通过子进程调用公开项目入口。
...
Python 依赖版本的唯一事实源是 `requirements.txt`，环境初始化的唯一实现是 `script/project/init_env.py`。
