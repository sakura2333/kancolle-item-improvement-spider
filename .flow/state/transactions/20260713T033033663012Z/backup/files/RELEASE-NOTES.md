...
- Reworked npm consumer-digest idempotency as stateless reconciliation: Data Build compares current consumer files with the highest stable npm package, reuses that version for identical data, and automatically repairs missing `improvement2`, dist-tags, or `online` without allocating another patch or depending on a previous Workflow run。
