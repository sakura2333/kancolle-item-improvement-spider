...
The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. Each release entry in `RELEASES.json` carries the consumer `contentDigest`; `data-build.yml` compares current consumer files with the highest stable npm package before allocating a version. An equal digest reuses that npm version and automatically reconciles missing variants, dist-tags, or `online`; only a different digest can allocate another patch. Existing registry tarballs are downloaded into the new immutable Candidate, while only missing variants are generated from the current equal-digest data. `release.yml` remains a manual recovery path for a frozen Candidate. Public main release is independent and does not publish npm data.
