...
- AI：只审核 `release/main-content.json` 投影出的公共代码与文档差异。
...
该命令从同一机械 Preview 生成完整 public snapshot，推送唯一 `public-beta/<releaseId>` 分支并写 `beta-receipt.json`。它不会修改 `VERSION`、`main`、npm dist-tag 或 `online`，也不会读取或覆盖 Stable 的 Migration ID。测试必须绑定输出的 branch、commit、tree 与 candidateSha256。
...
机械 Preview 会记录 `sourceCommit`、Git tree、main-content 配置摘要和 Candidate SHA-256，并生成 AI 审核模板。AI 在临时分支完成公开结构与文档净化后提交，再执行：
...
`release/main-content.json` 是唯一 main 内容声明。首次发布只对精确授权的 legacy tree 做一次完整替换；之后由 `STABLE-CONTENT-MANIFEST.json` 增量管理。`dist/**`、`.flow/**`、`script/**`、`tests/**`、`tools/**` 和本机状态不会进入 main。
