...
pyproject.toml、uv.lock、mise.toml（涉及 Python 构建或验收时）
```
...
- 工具版本：`mise.toml`（Python 3.14.6、uv 0.11.28）
- 依赖版本：`pyproject.toml` 与 `uv.lock`
- 环境初始化：`mise install && mise exec -- uv sync --locked`
- uv 不允许自行下载 Python
- `./flow check` 会先检查 Python 3.14、uv.lock 与锁定依赖；环境未就绪时会给出唯一下一步，不再把缺包误报为代码失败。
