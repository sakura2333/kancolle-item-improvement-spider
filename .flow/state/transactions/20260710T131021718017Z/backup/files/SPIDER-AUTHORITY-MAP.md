...
| WikiWiki 装备获取证据 | 原始证据为 `.flow/local/source-cache/**` 中带 `acquisition_source=external-browser-session-crawl` 的记录，离线解析结果为 `dist/data-pipeline/sources/wikiwiki-equipment-detail/**`；任务 code/名称只用于匹配与诊断，接受后的数字 `questKey` 可进入统一装备来源；同名舰明确形态可直接映射，裸名称必须由独立登记的 WikiWiki canonical 链接目标与 Start2 候选交叉验证 |
