...
def _write_beta_inventory(state_root: Path, config: dict, candidate: Path) -> Path:
    categories = config["stable"].get("categories", {})
    counts = {name: 0 for name in categories}
    counts["uncategorized"] = 0
    ...
    if counts["uncategorized"]:
        raise MainReleaseError("Beta 公开快照包含未分类文件；检查 release/main-content.json")
...
def _reset_worktree_to_candidate_snapshot(worktree: Path, candidate: Path) -> None:
    # Beta is an isolated, complete public snapshot.  It deliberately does not
    # interpret or mutate Stable's migration manifest because it never updates
    # main and never reuses the Stable managed-file set.
...
def _write_beta_receipt(
    ...
    *,
    ...
    content_manifest = state_root / "candidate" / "STABLE-CONTENT-MANIFEST.json"
    payload = {
        "schemaVersion": 1,
        "channel": "beta",
        ...
        "tree": tree,
        "remote": remote,
        "policy": "isolated-full-public-snapshot",
...
def prepare_beta(*, confirm: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="spider-public-beta-") as temp_name:
        ...
        try:
            _reset_worktree_to_candidate_snapshot(worktree, state_root / "candidate")
    ...
    inventory = _write_beta_inventory(state_root, config, state_root / "candidate")
    ...
    return {
        ...
        "receipt": str(receipt),
        "reviewInventory": str(inventory),
        "next": "test this exact Beta branch; formal main release remains unopened",
    }
