...
class MainBoundaryTest(unittest.TestCase):
    ...
    def test_beta_snapshot_replaces_entire_remote_tree_without_migration_reconciliation(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name) / "repo"
            root.mkdir()
            subprocess.run(["git", "init", "-q", "-b", "main"], cwd=root, check=True)
            ...
            (root / "STABLE-CONTENT-MANIFEST.json").write_text(
                json.dumps({"schemaVersion": 1, "project": "demo", "migrationId": "old", "managedFiles": ["old.txt"]}),
                encoding="utf-8",
            )
            subprocess.run(["git", "add", "-A"], cwd=root, check=True)
            subprocess.run(["git", "commit", "-qm", "base"], cwd=root, check=True)
            ...
            (candidate / "STABLE-CONTENT-MANIFEST.json").write_text(
                json.dumps({"schemaVersion": 1, "project": "demo", "migrationId": "new", "managedFiles": ["service/app.py"]}),
                encoding="utf-8",
            )
            with patch.object(main_release, "PROJECT_ROOT", root):
            ...
            self.assertEqual(tracked, ["STABLE-CONTENT-MANIFEST.json", "service/app.py"])
