...
from service.data_package.equipment_acquisition_crawl import reference_diagnostic_summary, run_full_crawl
...
class EquipmentAcquisitionTest(unittest.TestCase):
    ...
    def test_reference_diagnostics_report_resolved_and_unresolved_link_conflicts(self):
        catalog = ShipReferenceCatalog(
            Start2ShipUtils([
        ...
        for record in records:
            reference_issues.extend(resolve_record_references(
                record, ships=catalog, quests=QuestReferenceCatalog.empty()
            ))

        summary = reference_diagnostic_summary(
            records=records,
            reference_issues=reference_issues,
        )

        self.assertEqual(summary["resolvedLinkTargetConflictCount"], 1)
        self.assertEqual(summary["operatorStopReferenceCount"], 1)
        rows = summary["rows"]
        self.assertEqual(rows[0]["category"], "resolved-link-target-conflict")
        self.assertEqual(rows[0]["acceptedShip"], {"shipId": 288, "shipName": "翔鶴改"})
        self.assertEqual(rows[1]["category"], "operator-stop-reference")
        self.assertEqual(rows[1]["candidateShips"], [
            {"shipId": 1, "shipName": "赤城"},
            {"shipId": 2, "shipName": "加賀"},
        ])
