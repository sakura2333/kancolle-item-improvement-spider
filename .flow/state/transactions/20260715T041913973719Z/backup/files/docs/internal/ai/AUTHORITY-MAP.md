...
| 公开内容归属 | `release/public-content.json` 的项目内容注册表 |
| 冻结发布候选 | `.flow/state/release-transactions/**/candidate/manifest.json` 与同目录 `public/`、`candidate.zip` |
| 发布事务状态 | `.flow/state/release-transactions/**/status.json`、`result/**` 与 main release gate |
