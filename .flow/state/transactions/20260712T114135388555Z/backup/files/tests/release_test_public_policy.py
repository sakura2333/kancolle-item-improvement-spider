...
class PublicReleasePolicyTest(unittest.TestCase):
    ...
    def test_flow_and_maintenance_materials_are_internal_only(self):
        private = self.stable["privateCategories"]
        self.assertIn("script/**", private["internalControlPlane"])
        self.assertIn("tests/**", private["internalControlPlane"])
        self.assertIn("docs/internal/**", private["internalDocumentation"])
        self.assertIn("AGENTS.md", private["internalDocumentation"])
        self.assertIn(".flow/state/**", private["releaseTransactionState"])
    ...
    def test_main_release_review_gate_is_project_owned_and_code_only(self):
        self.assertEqual(self.stable["transactionRoot"], ".flow/state/release-transactions")
        ...
        self.assertIn(".flow/state/**", self.stable["privateCategories"]["releaseTransactionState"])
        ...
        self.assertIn("immutable-project-registry-candidate", collaboration)
    ...
    def test_public_snapshot_manifest_separates_public_set_from_internal_migration(self):
        ...
        self.assertNotIn("legacyPaths", content)
    ...
    def test_beta_and_stable_share_whitelist_snapshot_policy(self):
        self.assertEqual(self.stable["policy"], "project-content-registry-public-snapshot")
    ...
    def test_public_runtime_state_is_not_flow_state(self):
        ...
        offenders = []
        generated = set(self.stable.get("generated", []))
        for relative in stable_command._public_paths(ROOT, self.runtime):
