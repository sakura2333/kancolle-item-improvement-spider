...
class GithubActionPipelineTest(unittest.TestCase):
    ...
    def _workflow_job_blocks(text: str) -> dict[str, str]:
        ...
        matches = list(re.finditer(r"^  (?P<name>[A-Za-z0-9_-]+):\n", jobs_text, re.M))
        blocks: dict[str, str] = {}
    ...
    def test_jobs_using_repository_modules_checkout_before_execution(self):
        module_command = re.compile(
            r"(?:python3?|uv run --locked python) -m "
            r"(?:automation|service|configs|util)(?:\.|\s)"
        )
        for workflow_name, workflow in (
            ...
            for job_name, block in self._workflow_job_blocks(workflow).items():
                command = module_command.search(block)
                if command is None:
                    continue
                ...
                self.assertGreaterEqual(
                    checkout,
                    0,
                    f"{workflow_name}:{job_name} runs a repository module without checkout",
                )
                self.assertLess(
                    checkout,
                    command.start(),
                    f"{workflow_name}:{job_name} checks out after its first repository module call",
                )

    def test_resolve_source_bootstraps_repository_python_context(self):
        resolve_job = self._workflow_job_blocks(self.build)["resolve-source"]
        resolver_command = resolve_job.index(
            "python3 -m automation.compute.source_resolver"
        )
        self.assertIn("PYTHONPATH: ${{ github.workspace }}", resolve_job)
        self.assertLess(resolve_job.index("uses: actions/checkout@v6"), resolver_command)
        self.assertLess(resolve_job.index("uses: actions/setup-python@v6"), resolver_command)
        self.assertIn("python-version: '3.10'", resolve_job)
