...
LEGACY_TEMPLATE_CONFIG_PATH = Path("configs") / "templates" / "wikiwiki-crawler.json"
...
def _usage_error(message: str) -> dict:
    return result(
        ...
        [],
        [message],
        "执行 ./flow wikiwiki、./flow wikiwiki --full、./flow wikiwiki config 或 ./flow wikiwiki session",
        "无需回滚",
        2,
    )
...
def _source_config_template(root: Path) -> tuple[Path, str]:
    ...
    legacy = root / LEGACY_TEMPLATE_CONFIG_PATH
    if legacy.is_file():
        return legacy, "legacy-template"
...
def _parse(args: list[str]) -> tuple[str, bool, bool, int | None] | dict:
    values = list(args)
    action = "crawl"
    if values and values[0] in {"session", "config"}:
        action = values[0]
        values = values[1:]
