...
def _is_template(relative: str) -> bool:
    ...
    return value.startswith("configs/templates/") or ".template." in name or name.endswith(".template.json") or name.endswith(".default.json")
    ...
    for relative in paths:
        ...
        if not value.startswith("configs/") and value not in ALLOWED_CONFIG_OUTSIDE_CONFIGS:
            if any(_matches(value, pattern) for pattern in CONFIG_LIKE_PATTERNS):
                violations.append(f"项目配置默认值必须放在 configs/*.default.json 或 configs/templates/：{value}")
