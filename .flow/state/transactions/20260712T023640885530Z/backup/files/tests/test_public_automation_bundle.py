...
class PublicAutomationBundleTest(unittest.TestCase):
    ...
    def test_source_bundle_ready_lock_binds_manifest_and_collections(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            lock = write_ready_lock(bundle, manifest, collections=["wikiwiki", "akashi-list"])
            self.assertEqual(
    ...
    def test_verified_source_bundle_restores_process_audit_state(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            write_ready_lock(bundle, manifest, collections=["akashi-list"])
