...
_CONTROL_FILES = {"bundle-manifest.json", "source-bundle.lock.json"}
...
def write_manifest(output: Path, *, kind: str, project_id: str, commit: str, metadata: dict | None = None) -> dict:
    ...
    records = file_records(output, exclude=_CONTROL_FILES)
    payload = {
        "schemaVersion": 1,
        "kind": kind,
...
def verify_manifest(bundle: Path, *, expected_kind: str | None = None, expected_project: str | None = None) -> dict:
    ...
    actual = file_records(bundle, exclude=_CONTROL_FILES)
    if actual != declared:
...
def write_ready_lock(output: Path, manifest: dict, *, collections: list[str] | None = None) -> dict:
    manifest_path = output / "bundle-manifest.json"
    if not manifest_path.is_file():
        raise BundleError("bundle manifest must exist before ready lock is written")
    payload = {
        "schemaVersion": 1,
        "status": "ready",
    ...
    (output / "source-bundle.lock.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    return payload


def verify_ready_lock(bundle: Path, manifest: dict) -> dict:
    lock_path = bundle / "source-bundle.lock.json"
    if not lock_path.is_file():
        raise BundleError("source bundle ready lock is missing")
    payload = json.loads(lock_path.read_text(encoding="utf-8"))
    if payload.get("schemaVersion") != 1 or payload.get("status") != "ready":
        raise BundleError("source bundle ready lock is invalid")
    expected = {
        "projectId": manifest.get("projectId"),
        "kind": manifest.get("kind"),
        "commit": manifest.get("commit"),
        "contentHash": manifest.get("contentHash"),
        "manifestSha256": sha256_file(bundle / "bundle-manifest.json"),
    }
    for key, value in expected.items():
        if payload.get(key) != value:
            raise BundleError(f"source bundle ready lock mismatch: {key}")
    collections = payload.get("collections")
    if not isinstance(collections, list) or any(not isinstance(item, str) for item in collections):
        raise BundleError("source bundle ready lock collections are invalid")
    return payload
