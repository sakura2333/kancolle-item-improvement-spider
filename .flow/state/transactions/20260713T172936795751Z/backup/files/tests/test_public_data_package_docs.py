...
class PublicDataPackageDocumentationTest(unittest.TestCase):
    ...
    def test_lifecycle_pins_workflow_commit_for_frozen_source_bundles(self) -> None:
        text = DATA_LIFECYCLE.read_text(encoding="utf-8")
        self.assertIn("Data Build 始终使用本次 Workflow 固定的 `main` Commit", text)
        self.assertIn("`${{ github.sha }}`", text)
        self.assertIn("每次执行都选择当时最后一个成功的 Acquire", text)
