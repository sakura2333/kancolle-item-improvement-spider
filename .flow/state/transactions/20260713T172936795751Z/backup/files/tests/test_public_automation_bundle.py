...
class PublicAutomationBundleTest(unittest.TestCase):
    ...
    def test_acquire_prunes_retired_akashi_image_cache_only(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            cache = root / ".spider/local/source-cache"
            retired_useitem = cache / "cache/useitem/88.png"
            retired_equip = cache / "cache/equip/160.png"
            official_useitem = cache / "cache/official/useitem/88.png"
            encoded_equip = cache / "cache/official/equip/webp-q93-a100-m6-exact/160.webp"
            for path, data in (
                (retired_useitem, b"old-useitem"),
                (retired_equip, b"old-equip"),
                (official_useitem, b"official-useitem"),
                (encoded_equip, b"official-equip-webp"),
            ):
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(data)
            meta = {
                "cache/useitem/88.png": {"url": "https://akashi.invalid/useitem"},
                "cache/equip/160.png": {"url": "https://akashi.invalid/equip"},
                "cache/official/useitem/88.png": {"url": "https://official.invalid/useitem"},
                "cache/official/equip/webp-q93-a100-m6-exact/160.webp": {
                    "url": "derived-local"
                },
            }
            (cache / "_meta.json").write_text(json.dumps(meta), encoding="utf-8")

            result = prune_retired_image_cache(root)

            self.assertEqual(result, {"removedFiles": 2, "removedMetadata": 2})
            self.assertFalse(retired_useitem.exists())
            self.assertFalse(retired_equip.exists())
            self.assertTrue(official_useitem.exists())
            self.assertTrue(encoded_equip.exists())
            remaining = json.loads((cache / "_meta.json").read_text(encoding="utf-8"))
            self.assertEqual(
                set(remaining),
                {
                    "cache/official/useitem/88.png",
                    "cache/official/equip/webp-q93-a100-m6-exact/160.webp",
                },
            )

    def test_acquire_prefetches_kcquests_as_a_required_build_input(self):
        records = [{"equipmentId": 1}]
        with (
            ...
            patch(
                "automation.acquire.cli.acquire_required_assets",
                return_value={"equipmentCount": 1, "useitemCount": 2},
            ) as acquire_assets,
            patch("automation.acquire.cli.prefetch_source") as prefetch,
        ):
            statuses = acquire_non_wikiwiki_sources()

        acquire_assets.assert_called_once_with(records)
        ...
        self.assertEqual(statuses["official-assets"], "ready:1:2")
    ...
    def test_non_publish_candidate_binds_source_content_without_requiring_same_commit(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root, commit = self._repo(Path(temp_name))
            ...
            self.assertNotEqual(source_manifest["commit"], result["candidate"]["commit"])
