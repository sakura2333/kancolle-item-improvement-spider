...
def _unclassified_unknown_top_level(root: Path, top: str, config: dict) -> list[str]:
    path = root / top
    relative = path.relative_to(root).as_posix()
    if path.is_symlink():
        return [] if _private_override_for(relative, config) else [relative]
    if path.is_file():
        return [] if _private_override_for(relative, config) else [relative]
    ...
    for current, directories, files in os.walk(path, topdown=True, followlinks=False):
        ...
        for name in directories:
            ...
            if item.is_symlink():
                if not _private_override_for(item_relative, config):
                    findings.append(item_relative)
                continue
            if _private_override_for(item_relative, config):
                continue
...
def _top_level_guard(root: Path, config: dict) -> None:
    ...
    unclassified = [
        relative
        for top in unknown
        for relative in _unclassified_unknown_top_level(root, top, config)
    ]
    if unclassified:
        raise StableReleaseError(
            "项目顶级内容包含未登记归属的权威文件：\n"
            + "\n".join(f"- {value}" for value in unclassified)
        )
...
def _candidate_records(candidate: Path, managed_files: list[str] | None = None) -> list[dict]:
    if managed_files is None:
        paths = [
            ...
            and path.relative_to(candidate).parts[0] != ".git"
        ]
    else:
        paths = []
        for value in sorted(managed_files):
            relative = _safe_relative(value)
            path = candidate / relative
            if path.is_symlink() or not path.is_file():
                raise StableReleaseError(f"Candidate managed file 无效：{value}")
            paths.append(path)
    return [
        {
