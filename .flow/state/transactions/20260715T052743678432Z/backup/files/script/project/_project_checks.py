...
from script.project.directory_governance import verify_directory_governance
...
REQUIRED_PATHS = (
    ...
    PROJECT_ROOT / "docs/internal/ai/HARD-RULES.md",
    PROJECT_ROOT / "docs/internal/ai/AUTHORITY-MAP.md",
    PROJECT_ROOT / "docs/internal/ai/START.md",
    ...
    PROJECT_ROOT / "release" / "public-content.json",
    PROJECT_ROOT / "automation" / "acquire" / "cli.py",
    PROJECT_ROOT / "automation" / "compute" / "cli.py",
    PROJECT_ROOT / "automation" / "release" / "cli.py",
    PROJECT_ROOT / "service",
    PROJECT_ROOT / "automation" / "release" / "generated_state" / "artifact.py",
    PROJECT_ROOT / "util",
    PROJECT_ROOT / "service" / "improvement" / "model.py",
    PROJECT_ROOT / "service" / "data_package" / "improvement_record.py",
    ...
    PROJECT_ROOT / "script" / "project" / "public_content.py",
    PROJECT_ROOT / "script" / "project" / "public_content_audit.py",
...
def compile_python_sources() -> None:
    run(
        [sys.executable, "-m", "compileall", "-q", "automation", "configs", "service", "util", "script"],
        cwd=PROJECT_ROOT,
        env=project_env(),
    )
...
def run_basic_checks(*, include_generated_state: bool = True) -> None:
    ...
    verify_directory_governance(PROJECT_ROOT)
    try:
        from script.project.runtime import load
        inspect_main_boundary(PROJECT_ROOT, load()["stable"])
    except MainBoundaryError as exc:
