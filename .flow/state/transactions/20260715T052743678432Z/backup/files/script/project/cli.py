...
from script.project import candidate_verify, push_command, quality_command, run_command, smoke_command, stable_command, status_command, update_package_command, wikiwiki_command
from script.project.local_runtime_migration import migrate_for_project_command
...
def flow_handler(root: Path, args: list[str], adapter_config: dict | None = None, loader=None) -> dict:
    ...
    if command not in {"verify-candidate", "update-package"}:
        migrate_for_project_command(root)
    config = load_runtime()
    handlers = {
        ...
        "doctor": _doctor,
        "update-package": update_package_command.run,
