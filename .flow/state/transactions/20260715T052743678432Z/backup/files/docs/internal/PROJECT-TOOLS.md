...
npm 双制品由公开 `automation/release/**` 管理：Data Build 直接从当前目录与 Registry tarball 的实际 consumer 文件重算 canonical 与 `improvement2` 两个稳定身份。canonical 相同固定复用 npm 已有版本，兼容投影必须匹配自身 digest；版本号、时间戳和发布历史不参与身份。本次 Build 自动补齐缺失变体、dist-tag 与 `online`；只有 canonical 数据身份变化才分配新 patch。`release.yml` 仍可对冻结 Candidate 做补偿。Stable main 发布不构建或发布 npm。
