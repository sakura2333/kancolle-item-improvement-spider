...
def _candidate_from_argument(value: str) -> Candidate:
    ...
    if not sidecar.is_file():
        raise UpdateError(f"缺少 Artifact Sidecar：{sidecar}")
...
def _select(root: Path, config: dict, package_arg: str | None = None) -> tuple[Candidate | None, list[str]]:
    ...
    if package_arg:
        ...
        inbox, _ = _roots(root, config)
        inbox.mkdir(parents=True, exist_ok=True)
        candidates = []
        for sidecar in sorted(inbox.glob("*.flow.json")):
            try:
                candidates.append(_candidate_from_sidecar(sidecar))
            except UpdateError as exc:
                current_notes.append(f"无效：{sidecar.name}: {exc}")
    compatible: list[Candidate] = []
    for item in candidates:
        if item.identity.get("packageType") != "update":
            continue
        if item.project != project:
            current_notes.append(f"其他项目：{item.path.name}: {item.project}")
            continue
        if item.from_version != version:
            current_notes.append(f"基线不匹配：{item.path.name}: {item.from_version} -> {item.to_version}")
            continue
        compatible.append(item)
    if not compatible:
        return None, current_notes
    compatible.sort(key=lambda item: (_version_key(item.to_version), item.path.stat().st_mtime), reverse=True)
    top_version = compatible[0].to_version
    same = [item for item in compatible if item.to_version == top_version]
    if len(same) != 1:
        raise UpdateError("同一目标版本存在多个候选，禁止猜测：\n" + "\n".join(f"- {item.path.name}" for item in same))
    return same[0], current_notes
...
def _apply_action(root: Path, args: list[str], config: dict):
    ...
    try:
        ...
        destination_sidecar = applied / candidate.sidecar.name
