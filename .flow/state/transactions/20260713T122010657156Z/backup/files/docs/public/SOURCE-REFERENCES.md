...
| `official-useitem-card` | 舰 C 官方消耗品卡 | `https://<game-server>/kcs2/resources/useitem/card[_]/{id}.png` | `service/data_package/official_assets.py` | 按 Start2 useitem ID 获取官方 PNG | 图片正式来源，不参与改修事实选举 |
...
普通文件缓存默认有效期为 7 天；官方 useitem PNG 与装备 `slot/card` 原图通过 `download_pic()` 下载，图片缓存有效期统一为 180 天。装备原图保存在 Source Cache，随后确定性编码为 WebP quality 93。所有请求元数据写入：
...
- 装备官方原图按 Start2 `api_version` 保存在 `.spider/local/source-cache/cache/official/equip/{id}/{api_version}.png`，避免半年 TTL 掩盖资源版本变化；编码缓存位于 `.spider/local/source-cache/cache/equip/webp-q93-a100-m6-exact/{id}.webp`，发布投影保存在 `dist/data-pipeline/assets/equip/{id}.webp`，保持 390×390，固定 WebP quality 93、alpha quality 100、method 6。
- `api_mst_useitem` 提供消耗品 ID；Spider 按官方 `useitem/card` 与旧式 `card_` 路径获取 PNG，并发布到 `assets/useitem/{id}.png`。
