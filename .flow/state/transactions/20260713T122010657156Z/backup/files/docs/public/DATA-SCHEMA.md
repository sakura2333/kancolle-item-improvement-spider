...
`assets/equip/{id}.webp` is generated from the official 390×390 `slot/card` PNG without resizing. Encoding is fixed at WebP quality 93, alpha quality 100 and method 6. `assets/useitem/{id}.png` remains the official useitem PNG. IDs are validated against Start2; Akashi image URLs are not used.
