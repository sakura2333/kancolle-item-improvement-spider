...
Akashi 路线转换完成后，由独立 official-assets 阶段按最终引用 ID 获取官方 useitem/equip 原图并保存到 Source Cache；图片缓存有效期为 180 天，npm 仅发布 useitem PNG 和装备 WebP 投影。
...
- 舰 C 官方 `kcs2/resources`：useitem 原始 PNG 与装备 `slot/card` 原图；装备原图在构建时编码为 390×390 WebP quality 93；
