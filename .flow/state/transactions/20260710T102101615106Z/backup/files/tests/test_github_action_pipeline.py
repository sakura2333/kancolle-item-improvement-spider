...
class GithubActionPipelineTest(unittest.TestCase):
    ...
    def test_uses_python_314_and_locked_uv_environment(self):
        self.assertIn("jdx/mise-action@v4.2.0", self.text)
        self.assertIn("mise exec -- uv sync --locked", self.text)
        self.assertNotIn("astral-sh/setup-uv", self.text)
