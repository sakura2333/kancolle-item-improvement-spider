...
class CacheFreshnessTest(unittest.TestCase):
    ...
    def test_text_cache_ttl_is_22_hours(self):
        self.assertEqual(settings.LOCAL_CACHE_TTL_SECONDS, 46 * 60 * 60)

    def test_strict_fetch_reuses_cache_within_22_hours_without_network(self):
        url = "https://example.test/data.json"
        self._write_cache(url, age_seconds=21 * 60 * 60 + 59 * 60)
    ...
    def test_strict_fetch_revalidates_once_22_hour_ttl_expires(self):
        url = "https://example.test/data.json"
        self._write_cache(url, age_seconds=settings.LOCAL_CACHE_TTL_SECONDS + 1)
        response = FakeResponse(status_code=304, headers={"ETag": '"fixture"'})
    ...
    def test_strict_download_pic_reuses_cache_within_30_days_without_network(self):
        ...
        relative_path = "cache/images/1.png"
