...
def _package_dirs(root: Path, config: dict) -> list[Path]:
    ...
    candidates = [root / ".flow" / "packages", inbox]
    for value in values:
    ...
    for folder in _package_dirs(root, config):
        ...
        for sidecar in sorted(folder.glob("*.zip.flow.json")) + sorted(folder.glob("*.flow.json")):
            try:
                ...
                notes.append(f"无效：{sidecar.name}: {exc}")
                continue
        ...
        for artifact in sorted(folder.glob("*.zip")):
            ...
            try:
                ...
                # Non Flow zip files may share the download directory; keep a short note only for project-looking names.
                if config["project"]["id"] in artifact.name:
                    notes.append(f"无效：{artifact.name}: {exc}")
                continue
            seen_artifacts.add(key)
            candidates.append(candidate)
...
def _compatible_candidates(root: Path, config: dict, package_arg: str | None = None) -> tuple[list[Candidate], list[str]]:
    ...
    if package_arg:
        ...
        candidates, notes = _discover_candidates(root, config)
...
def _select_chain(root: Path, config: dict) -> tuple[list[Candidate], list[str]]:
    compatible, notes = _compatible_candidates(root, config, None)
    if not compatible:
        return [], notes
    current_package_version = _baseline_package_version(root)
    by_from: dict[tuple[str, str], list[Candidate]] = {}
    for item in compatible:
        if item.package_version and item.package_version <= current_package_version:
            continue
        by_from.setdefault((item.base_identity["scheme"], item.base_identity["value"]), []).append(item)
    for values in by_from.values():
        values.sort(key=lambda item: (item.package_version, _version_key(item.to_version), item.path.stat().st_mtime))
    chain: list[Candidate] = []
    current_by_scheme: dict[str, str] = {}
    while True:
        found: list[Candidate] = []
        for scheme in sorted({item.base_identity["scheme"] for item in compatible}):
            if scheme not in current_by_scheme:
                current_by_scheme[scheme] = _identity_value(root, scheme, config)
            found.extend(by_from.get((scheme, current_by_scheme[scheme]), []))
        if not found:
            break
        found.sort(key=lambda item: (item.package_version, _version_key(item.to_version), item.path.stat().st_mtime))
        same = [item for item in found if item.package_version == found[0].package_version and item.base_identity == found[0].base_identity]
        if len(same) > 1:
            raise UpdateError("同一 packageVersion/fromHash 存在多个候选，禁止猜测：\n" + "\n".join(f"- {item.path.name}" for item in same))
        candidate = found[0]
        chain.append(candidate)
        current_by_scheme[candidate.target_identity["scheme"]] = candidate.target_identity["value"]
    if not chain:
        higher = [item for item in compatible if not item.package_version or item.package_version > current_package_version]
        if higher:
            notes.append(f"发现 {len(higher)} 个更高序号更新，但缺少可接续当前 contentHash 的前置包")
    return chain, notes
...
def _stage_project_owned(root: Path, config: dict) -> list[str]:
    ...
    if paths:
        for start in range(0, len(paths), 200):
            subprocess.run(["git", "add", "-A", "--", *paths[start:start + 200]], cwd=root, check=True)
...
def _trusted_project_python(root: Path) -> Path:
    explicit = os.getenv("FLOW_PROJECT_PYTHON", "").strip()
    candidates = [Path(explicit).expanduser()] if explicit else []
    if os.name == "nt":
        candidates.append(root / ".venv" / "Scripts" / "python.exe")
    else:
        candidates.extend((root / ".venv" / "bin" / "python", root / ".venv" / "bin" / "python3"))
    for candidate in candidates:
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    raise UpdateError(
        "项目候选验证环境不可用；请先在真实项目根目录执行 "
        "python3 script/project/init_env.py"
    )
    ...
    for item in command:
        if item == "{python}":
            ...
            if project_python is None:
                project_python = str(_trusted_project_python(trusted_root))
            rendered.append(project_python)
        elif item == "{project}":
...
def _sync_baseline_if_current_is_known_target(root: Path, config: dict, candidates: list[Candidate]) -> tuple[dict | None, list[str]]:
    notes: list[str] = []
    current_by_scheme: dict[str, str] = {}
    matches: list[Candidate] = []
    for item in candidates:
        scheme = item.target_identity["scheme"]
        if scheme not in current_by_scheme:
            current_by_scheme[scheme] = _identity_value(root, scheme, config)
        if current_by_scheme[scheme] == item.target_identity["value"]:
            matches.append(item)
    if not matches:
        return None, notes
    matches.sort(key=lambda item: (item.package_version, _version_key(item.to_version), item.path.name), reverse=True)
    best = matches[0]
    same_best = [item for item in matches if item.package_version == best.package_version and item.target_identity == best.target_identity]
    if len({_candidate_fingerprint(item) for item in same_best}) > 1:
        raise UpdateError("当前内容身份匹配多个不同 package target，不能自动 package sync")
    current_version = _baseline_package_version(root)
    if best.package_version and best.package_version < current_version:
        return None, notes
    state, commit, created = _write_synced_flow_baseline(
        root, best, config, reason="current-content-matches-known-target"
    )
    notes.append(
        f"package sync：当前内容匹配 {best.path.name} target，baseline 已同步到 {best.package_version or '?'}"
    )
    if created:
        notes.append(f"package sync 提交：{commit[:12]}")
    return state, notes
...
def _apply_action(root: Path, args: list[str], config: dict):
    ...
    if package_arg is None and "--yes" in args:
        compatible, notes = _compatible_candidates(root, config, None)
        _sync_state, sync_notes = _sync_baseline_if_current_is_known_target(root, config, compatible)
        notes.extend(sync_notes)
        chain, chain_notes = _select_chain(root, config)
        notes.extend(chain_notes)
        if not chain:
            if sync_notes:
                return _result("成功", "已是目标内容身份，baseline 已同步", notes, [], "继续当前工作", "无需回滚")
            return _result("成功", "没有可应用更新", notes, [], "继续当前工作", "无需回滚")
        completed = [*notes, f"自动更新链：{len(chain)} 个包"]
        incomplete: list[str] = []
        last_result = None
        for candidate_item in chain:
            one = _apply_action(root, ["--package", str(candidate_item.path), "--yes", "--non-interactive"], config)
            last_result = one
            completed.append(
                f"应用 {candidate_item.package_version or '?'}："
                f"{candidate_item.base_identity['value'][:12]} → {candidate_item.target_identity['value'][:12]}"
            )
            completed.extend(str(item) for item in one.get("completed", []) if str(item).startswith("更新提交"))
            incomplete.extend(str(item) for item in one.get("incomplete", []))
        return _result(
            "成功",
            f"自动更新完成：{chain[-1].package_version or chain[-1].to_version}",
            completed,
