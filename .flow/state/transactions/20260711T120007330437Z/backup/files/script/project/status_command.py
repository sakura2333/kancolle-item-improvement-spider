...
def run(root: Path, args: list[str], config: dict, loader=None):
    ...
    if local_path.is_file():
        ...
        project_value = local.get("projectPath")
        if project_value:
            project_path = Path(str(project_value)).expanduser().resolve()
            if project_path == root.resolve():
                completed.append("Project Path 与当前目录一致")
            else:
                incomplete.append(f"Project Path 不一致：{project_path}")
        workspace_value = local.get("workspaceRoot")
        if workspace_value:
        ...
        download_value = local.get("downloadRoot")
        if download_value:
            download = Path(str(download_value)).expanduser().resolve()
            if download.is_dir():
    ...
    wikiwiki = wikiwiki_receipt_summary(root)
    if wikiwiki["ready"]:
        completed.append(wikiwiki["line"])
        completed.extend(str(item) for item in wikiwiki["details"])
    else:
        incomplete.append(wikiwiki["line"])
        incomplete.extend(str(item) for item in wikiwiki["details"])
    if generated_count:
    ...
    if code_count:
        next_step = "./flow check --profile quick"
    elif wikiwiki["ready"]:
        next_step = "./flow run（将复用 ready 的 WikiWiki source receipt）或继续开发"
    else:
        next_step = str(wikiwiki["next"])
