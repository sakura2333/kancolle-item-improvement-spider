...
The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. `data-build.yml` freezes both tarballs, versions, hashes, and required dist-tags in one immutable Candidate Bundle, verifies and uploads that bundle, then automatically publishes or reconciles `latest` and `improvement2` and updates `online` when the release plan requires publication. `release.yml` is the manual recovery path for the same frozen Candidate. Public main release is independent and does not publish npm data.
