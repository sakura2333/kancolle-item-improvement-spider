...
PUBLIC_COMMANDS = (
    "status", "check", "run", "wikiwiki", "push", "beta", "stable", "package", "update", "rollback",
...
def _validate_public_options(command: str, options: dict[str, Any], capabilities: dict[str, Any]) -> dict[str, Any]:
    ...
    if command not in {"run", "push", "beta", "stable"} and options["target"] is not None:
        raise ValueError("--target 不适用于该命令")
    if command != "package" and options["output"] is not None:
        raise ValueError("--output 只适用于 package")
