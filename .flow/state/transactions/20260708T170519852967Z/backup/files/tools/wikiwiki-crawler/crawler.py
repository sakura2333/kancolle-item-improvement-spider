...
DEFAULT_OUTPUT = Path(".flow/local/wikiwiki-crawler")
DEFAULT_RAW_ROOT = Path(".flow/local/source-cache")
DEFAULT_START2 = Path("data/start2_data/api_mst_slotitem.json")
DEFAULT_CATALOG = Path(".flow/local/wikiwiki-crawler/catalog/equipment-pages.json")
DEFAULT_CATALOG_DIR = Path(".flow/local/wikiwiki-crawler/catalog")
DEFAULT_SOURCE_RECEIPT = Path(".flow/local/wikiwiki-crawler/source-receipt.json")
DETAIL_ROLES = {
...
def load_browser_session_config(path: Path) -> dict[str, Any]:
    try:
        ...
        if base_path.is_file() and base_path.resolve() != path.resolve():
...
def validate_config(value: Any) -> dict[str, Any]:
    ...
    defaults = {
        ...
        "dailyLimit": 30,
        "maxAgeDays": 20,
        "catalogMaxAgeHours": 22,
        "curlPath": "curl",
        "browserProfileDir": ".flow/local/wikiwiki-browser-profile",
...
def _playwright_context(config: dict[str, Any], project: Path, *, headless: bool | None = None):
    try:
        ...
        raise RuntimeError(
            ...
            "tools/wikiwiki-crawler/requirements.txt` and `python -m playwright install chromium`"
        ) from exc
    ...
    if catalog_path.is_file():
        ...
        if counts.get("invalid", 0):
            print(_red_warning(
                ...
                "run catalog and inspect .flow/local/wikiwiki-crawler/catalog/equipment-name-matches.json"
            ), file=sys.stderr)
    else:
...
def crawl_command(args: argparse.Namespace) -> int:
    ...
    if daily_limit_value is None:
        daily_limit_value = config.get("dailyLimit", 30)
