...
- Added npm consumer-digest idempotency: Data Build reads the highest stable package `RELEASES.json` before allocating a version, rejects a second patch for the same `contentDigest`, and records registry/current/online identities in the frozen release plan。
