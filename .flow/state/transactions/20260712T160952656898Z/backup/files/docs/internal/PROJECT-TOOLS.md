...
npm 双制品由公开 `automation/release/**` 管理：Data Build 在版本规划前读取 npm 最高正式版本 `RELEASES.json` 的消费者 `contentDigest`；相同 digest 已发布时禁止分配新 patch，若 `online` 落后则必须用原 Candidate 补偿。只有 digest 尚未发布、Candidate 验证通过且发布计划为真时，才自动把 canonical 版本发布到 `latest`、schema-3 兼容版本发布到 `improvement2`，并更新 `online`；`release.yml` 只对既有 Candidate 做补偿或对账。Stable main 发布不构建、发布或对账 npm。
