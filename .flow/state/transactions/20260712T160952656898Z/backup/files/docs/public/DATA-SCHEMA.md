...
The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. Each release entry in `RELEASES.json` carries the consumer `contentDigest`; `data-build.yml` reads the highest stable npm package entry before allocating a new version, so identical consumer data cannot be published again under another patch. The frozen release plan records the current digest, npm registry head, and online baseline. The workflow then freezes both tarballs, versions, hashes, and required dist-tags in one immutable Candidate Bundle, verifies and uploads that bundle, and publishes or reconciles `latest`, `improvement2`, and `online` only when allowed. `release.yml` is the manual recovery path for the same frozen Candidate. Public main release is independent and does not publish npm data.
