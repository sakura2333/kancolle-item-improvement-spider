...
from automation.release.generated_state.config import (
...
def _verify_automation_contract() -> None:
    ...
    expected_human = ["status", "check", "smoke", "run", "wikiwiki", "run-wikiwiki-source", "push", "beta", "stable", "package", "update-package", "update", "rollback"]
    ...
    for required in ("script/**", "tests/**", "docs/internal/**"):
        if required not in stable["internalOnly"]:
            raise ProjectCommandError(f"Stable 内网隔离缺少 {required}")
    if ".flow/**" not in stable["forbidden"]:
        raise ProjectCommandError("Public Snapshot 必须整体禁止 .flow/**")
    private_categories = stable.get("privateCategories", {})
    if ".flow/state/**" not in private_categories.get("releaseTransactionState", []):
        raise ProjectCommandError("Release Transaction State 未登记为内部动态状态")
    if ".flow/project.json" not in private_categories.get("internalControlPlane", []):
        raise ProjectCommandError("Flow 项目配置未登记为内部控制面")
    if ".github/workflows/**" not in stable["include"]:
        raise ProjectCommandError("Stable 必须包含公开产品数据流水线")
    for workflow in ("source-acquire.yml", "data-build.yml", "release.yml"):
        if not (PROJECT_ROOT / ".github" / "workflows" / workflow).is_file():
            raise ProjectCommandError(f"Stable 缺少公开流水线：{workflow}")
