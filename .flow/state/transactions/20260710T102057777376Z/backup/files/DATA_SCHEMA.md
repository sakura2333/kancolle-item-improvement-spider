...
The source-side projection under `data/sources/equipment-sources/` stores input hashes and added/changed/removed deltas. It is generated state and is not part of code update identity.
...
`data/sources/history/baseline/*.json` is the one-time full stock created when the feature is introduced. `current/*.json` stores the latest successful semantic snapshot, while `changes/*.nedb` appends only `added`, `removed`, `modified`, and `reappeared` events. Failed or partial source results are not observed.

History uses the broad comparison identity:

`itemId + shipId + capability + updateTargetItemId`

`data/sources/reliability/summary.json` exposes advisory relative weights derived from pairwise agreement, peer consensus, and sufficiently mature historical corroboration. The file explicitly sets `applyToCanonicalElection=false`; it is not a consumer contract and does not change dataset authority.
