...
class StablePreviewWorkspaceTest(unittest.TestCase):
    ...
    def test_generated_and_local_changes_are_allowed_and_bound(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            (root / "data/raw_data/cache.html").write_text("local\n", encoding="utf-8")

            after = stable_command._workspace_source_state(root)

            self.assertNotEqual(before["generatedStateSha256"], after["generatedStateSha256"])
            ...
            selected = stable_command._public_paths(
                ...
                {
                    "stable": {
                        "include": ["service/**", "data/output/**"],
                        "internalOnly": ["data/raw_data/**"],
                        "required": ["service/app.py"],
                        "generated": [],
                    }
                },
            )
            self.assertIn("data/output/new.json", selected)
            self.assertNotIn("data/raw_data/cache.html", selected)
    ...
    def test_load_latest_detects_generated_state_drift(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = self.make_repo(Path(temp_name))
            config = {
                "stable": {
            ...
            state_root = root / ".flow/state/stable" / release_id
            ...
            (root / ".flow/state/stable/latest.json").write_text(
                json.dumps({"releaseId": release_id}), encoding="utf-8"
            )
            (root / "data/output/value.json").write_text('{"value":3}\n', encoding="utf-8")

            with self.assertRaisesRegex(
                stable_command.StablePreviewStaleError,
                "generated-state",
            ):
                stable_command._load_latest(root, config, require_npm=False)
