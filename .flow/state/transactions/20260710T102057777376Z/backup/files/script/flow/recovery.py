...
def _create(root: Path, args: list[str], config: dict):
    ...
    with tempfile.TemporaryDirectory(prefix="spider-recovery-") as temp_name:
        temp = Path(temp_name)
        ...
        manifest = {
            ...
            "files": [],
            "privateContent": any(kind != "tracked" for _, kind in files.values()),
        }
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
            ...
            if bundle_ok:
                archive.write(bundle, "git/project.bundle")
                manifest["files"].append({
