...
`data/sources/history/` 保存来源事实的时间变化，`data/sources/reliability/` 输出仅供分析的相对权重。两者都属于公开诊断数据，不属于 npm 消费接口，也不会改变正式数据来源。
...
`data/sources/reliability/summary.json` 依据以下信号给出平均值为 1 左右的建议权重：
...
执行 `./flow run` 后，人类结果会直接显示各来源的建议权重和置信度。完整 `data-validate` 日志还会逐来源记录：
