...
- `mise exec -- uv run --locked script/project/equipment_acquisition.py`（只读取 `data/raw_data/site_cache` 的离线装备获取方式诊断；关联 Start2 shipId 与 `kcQuests` 数字 `questKey`，并打印异常摘要）
