...
class PublicContentAuditTest(unittest.TestCase):
    ...
    def test_public_paths_exclude_local_config_by_project_registry(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            (root / "configs").mkdir()
            ...
            selected = stable_command._public_paths(
                ...
                {
                    "stable": {
                        "topLevelManaged": ["configs"],
                        "topLevelExternal": [],
                        "categories": {"runtime": ["configs/**"]},
                        "privateCategories": {"localRuntimeState": ["configs/*.local.*"]},
                        "privateOverrides": ["localRuntimeState"],
    ...
    def test_unknown_top_level_with_only_machine_state_is_ignored(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            (root / "service").mkdir()
            (root / "service/app.py").write_text("VALUE = 1\n", encoding="utf-8")
            (root / ".gpt/quality").mkdir(parents=True)
            (root / ".gpt/quality/latest.json").write_text("{}\n", encoding="utf-8")
            (root / "pojo/__pycache__").mkdir(parents=True)
            (root / "pojo/__pycache__/legacy.cpython-314.pyc").write_bytes(b"cache")

            selected = stable_command._public_paths(
                root,
                {
                    "stable": {
                        "topLevelManaged": ["service"],
                        "topLevelExternal": [],
                        ...
                        "privateOverrides": ["machineWorkspace"],
            ...
            self.assertEqual(selected, ["service/app.py"])

    def test_unknown_top_level_with_authoritative_file_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            (root / "service").mkdir()
            (root / "service/app.py").write_text("VALUE = 1\n", encoding="utf-8")
            (root / "pojo").mkdir()
            (root / "pojo/improvement.py").write_text("VALUE = 1\n", encoding="utf-8")

            with self.assertRaisesRegex(stable_command.StableReleaseError, "pojo/improvement.py"):
                stable_command._public_paths(
                    root,
                    {
                        "stable": {
                            "topLevelManaged": ["service"],
