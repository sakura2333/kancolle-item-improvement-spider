...
- `PYTHONPATH=. .venv/bin/python script/project/equipment_acquisition.py`（只读取 `data/raw_data/site_cache` 的离线装备获取方式诊断；关联 Start2 shipId 与 `kcQuests` 数字 `questKey`，并打印异常摘要）
...
公开 `main` 不发布 Raw Cache。严格构建只有在 `data/raw_data/site_cache/_meta.json` 存在时才重建 WikiWiki 装备获取快照；干净公开 checkout 会先校验 `data/sources/wikiwiki-equipment-detail/` 的完整性再复用。Raw Cache 存在但解析失败时不得回退旧快照。
