...
class StablePreviewWorkspaceTest(unittest.TestCase):
    ...
    def test_generated_and_local_changes_are_allowed_but_not_public_bound(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            (root / ".spider/local/source-cache").mkdir(parents=True)
            (root / ".spider/local/source-cache/cache.html").write_text("local\n", encoding="utf-8")
            ...
            selected = stable_command._public_paths(
                root,
                {
                    "stable": {
                        "include": ["service/**"],
                        "internalOnly": [".spider/local/source-cache/**"],
            ...
            self.assertNotIn(".spider/local/source-cache/cache.html", selected)
