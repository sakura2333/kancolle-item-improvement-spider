...
该命令直接消费与 Stable 完全相同的机械 Public Snapshot，推送唯一 `public-beta/<releaseId>` 分支并写 `beta-receipt.json`。它不会修改 `VERSION`、`main`、npm dist-tag 或 `online`，也不会修改 main、版本或发布状态。测试必须绑定输出的 branch、commit、tree、candidateContentSha256、candidateArchiveSha256 与 publicIsolation。命令同时生成已净化的 `beta-ai-review.zip`，该包只使用相对文件名。
...
机械 Preview 会记录 `sourceCommit`、Git tree、Public 配置摘要、`candidateContentSha256`、确定性 `candidateArchiveSha256`、例外清单哈希和 Public Isolation 结果，并生成 AI 审核模板。AI 在临时分支完成公开结构与文档净化后提交，再执行：
...
`release/public-content.json` 是 Beta 与 Stable 共用的唯一 Public Snapshot 声明；`release/public-exceptions.json` 只登记无法物理隔离的最小公共语义。构建器只从白名单复制文件；`PUBLIC-CONTENT-MANIFEST.json` 只记录公开文件集合，不携带内部 Migration ID。首次迁移和 legacy 清单兼容策略只留在 dev 控制面。`dist/**`、`.flow/**`、`script/**`、`tests/**`、`tools/**` 和本机状态不会进入 main。
...
完整隔离、例外和 AI 兜底规则见 `docs/internal/PUBLIC-ISOLATION.md`。
