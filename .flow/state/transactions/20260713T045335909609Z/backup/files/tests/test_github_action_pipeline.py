...
class GithubActionPipelineTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            if head_version is not None:
                ...
                head = {
                    ...
                    "improvement2ContentDigest": (
                        improvement2_digest
                        if improvement2_digest is not None
                        else (head_digest if improvement2_exists else None)
                    ),
                    "improvement2Tag": (
    ...
    def test_registry_head_consumer_digest_is_read_before_stateless_planning(self):
        ...
        self.assertIn("immutable npm registry conflict", registry)
    ...
    def test_same_consumer_digest_reuses_registry_head_and_reconciles_automatically(self):
        ...
        self.assertEqual(plan["reason"], "reconcile-already-published-consumer-data")
    ...
    def test_existing_improvement2_digest_mismatch_is_an_immutable_registry_conflict(self):
        digest = "a" * 64
        completed, plan = self._run_stateless_plan(
            current_digest=digest,
            online_digest=digest,
        ...
        self.assertIn("immutable npm registry conflict", completed.stderr)
    ...
    def test_new_consumer_digest_allocates_next_patch_from_registry_only(self):
        completed, plan = self._run_stateless_plan(
            current_digest="c" * 64,
            online_digest="c" * 64,
            online_version="0.5.10",
    ...
    def test_reconciliation_hydrates_existing_registry_tarballs_into_candidate(self):
        ...
        self.assertIn("npm pack", prepare)
        self.assertIn('version not in versions', prepare)
        self.assertIn('artifact.update(', prepare)
        self.assertIn('result.update(', prepare)
