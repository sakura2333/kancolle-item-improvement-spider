...
def _improvement_assistant_reverse_lines(root: Path, *, limit: int = 12) -> list[str]:
    ...
    expected = int(payload.get("expectedEquipmentCount") or payload.get("threshold") or 3)
    mismatch_count = int(payload.get("equipmentCountMismatchShipDayCount") or 0)
    over_count = int(payload.get("overThresholdShipDayCount") or 0)
    max_count = int(payload.get("maxEquipmentCount") or 0)
    lines = [
        "改修秘书舰反查："
        f"期望装备数={expected}，"
        f"不等于期望的舰/日={mismatch_count}，"
        f"超过期望的舰/日={over_count}，"
        f"最大装备数={max_count}，"
        "报告=dist/data-pipeline/sources/improvement-assistant-reverse/assistant-day-reverse-index.md"
    ]
    rows = payload.get("equipmentCountMismatches") if isinstance(payload.get("equipmentCountMismatches"), list) else []
    for row in rows[: max(limit, 0)]:
        ...
        lines.append(
            ...
            f"expected={row.get('expectedEquipmentCount', expected)} "
