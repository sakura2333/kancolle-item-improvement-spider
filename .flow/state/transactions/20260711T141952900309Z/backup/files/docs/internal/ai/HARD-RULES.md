...
宿主机绝对路径文本不是稳定身份。测试和运行时不得用完整绝对路径字符串或两个 `resolve()` 结果的相等性表达业务契约。

- 项目输出布局用“相对于项目根的 POSIX 路径”表达和断言；
- 路径逃逸与目录边界使用解析后的包含关系检查；
- 已存在文件或目录是否为同一实体使用 `samefile()`；
- `/var`、`/private/var`、`/tmp`、挂载点、大小写和用户目录展示形式不得进入稳定测试契约。
