...
def _write_review_delta(
    ...
    *,
    ...
    categories = stable.get("categories", {})
    retired_patterns = stable.get("reviewRetiredPublic", [])
    changes: list[dict[str, Any]] = []
    counts = {name: 0 for name in categories}
    counts["retiredPublic"] = 0
    ...
    for raw in completed.stdout.splitlines():
        ...
        if (
            ...
            and status == "D"
            and any(stable_command._matches(path, pattern) for pattern in retired_patterns)
        ):
            category = "retiredPublic"
        counts[category] += 1
        changes.append({"status": status, "path": path, "category": category})
        if category == "uncategorized":
            uncategorized.append(f"{status}\t{path}")
    if uncategorized:
        raise MainReleaseError(
            "公开候选包含未分类文件；检查项目内容注册表：\n"
            + "\n".join(f"- {value}" for value in uncategorized[:50])
        )
