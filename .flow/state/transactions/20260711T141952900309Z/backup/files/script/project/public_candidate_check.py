...
def inspect_candidate(worktree: Path, config: dict) -> dict[str, Any]:
    ...
    if manifest.get("schemaVersion") != 2 or manifest.get("project") != config["project"]["id"]:
    ...
    boundary = None
    audit = None
    if stable.get("categories"):
        try:
            from script.project.main_boundary import inspect_main_boundary
            from script.project.public_content_audit import inspect_public_text
            boundary = inspect_main_boundary(worktree, stable)
            audit = inspect_public_text(worktree, stable)
        except Exception as exc:
            raise PublicCandidateError(f"AI 候选 Public Snapshot 边界无效：{exc}") from exc
    return {
        "manifest": manifest,
        "managedFiles": sorted(normalized),
        "mainBoundary": boundary,
        "publicContentAudit": audit,
    }
