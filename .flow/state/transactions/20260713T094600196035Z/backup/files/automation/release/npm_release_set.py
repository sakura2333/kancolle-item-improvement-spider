...
def _verify_tarball(
    ...
    *,
    ...
    with tarfile.open(tarball, "r:gz") as archive:
        ...
        entry = next(
            (
                ...
                if isinstance(item, dict) and item.get("version") == release_version
            ),
            None,
        )
    ...
    identity = inspect_consumer_tarball(tarball, variant=variant)
    content_digest = str(identity["contentDigest"])
    metadata_key = "improvement2ContentDigest" if compatibility else "contentDigest"
    declared = str((entry or {}).get(metadata_key) or "").strip().lower()
    if declared and declared != content_digest:
        raise ProjectCommandError(
            f"npm tarball {result['version']} {metadata_key} does not match actual consumer files"
        )
...
def build_release_set(
    ...
    *,
    ...
    current_digest = _verify_tarball(
        current_verify,
        schema_version=4,
        compatibility=False,
        release_version=str(current["version"]),
    )
    compatibility_digest = _verify_tarball(
        compatibility_verify,
        schema_version=3,
        compatibility=True,
        release_version=str(current["version"]),
    )
    current["contentDigest"] = current_digest
    compatibility["contentDigest"] = compatibility_digest
...
def hydrate_published_artifacts(
    ...
    current consumer data identity.  The frozen candidate must then contain the
    ...
    for artifact in artifacts:
        ...
        expected_digest = str(artifact.get("contentDigest") or "")
        if registry_digest != expected_digest:
            raise ProjectCommandError(
                f"immutable npm registry conflict: {version} actual consumer files do not "
                f"match the frozen {variant} identity"
            )
