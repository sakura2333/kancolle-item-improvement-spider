...
- Reworked npm consumer-digest idempotency as stateless reconciliation: Data Build compares current consumer files with the highest stable npm package, reuses that version for identical data, verifies that existing canonical and `improvement2` variants carry the same release digest, and automatically repairs missing variants, dist-tags, or `online` without allocating another patch or depending on a previous Workflow run。
- Replaced release-history digest trust with normalized identities computed from actual consumer files; canonical schema-4 and the `improvement2` schema-3 projection now use separate stable digests, excluding version, timestamp, manifest, changelog, release-history, and audit metadata。
