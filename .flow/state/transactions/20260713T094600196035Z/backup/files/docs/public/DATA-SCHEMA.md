...
The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. `data-build.yml` computes identities from actual consumer files with identity schema 2: canonical schema-4 and the schema-3 compatibility projection have separate digests. JSON and NeDB are canonically serialized; image bytes are hashed directly; version, timestamp, manifest, release-history and audit metadata are excluded. The canonical digest controls patch allocation. An existing `improvement2` tarball must match the current projection digest or the immutable Registry state is rejected. Matching data automatically reconciles missing variants, dist-tags, or `online`; only a different canonical digest can allocate another patch. Existing Registry tarballs are downloaded into the new immutable Candidate, while only missing variants are generated. `release.yml` remains a manual recovery path for a frozen Candidate.
