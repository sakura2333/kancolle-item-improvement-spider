...
from automation.release import npm_registry as npm_publish
...
class NpmPublishReconciliationTest(unittest.TestCase):
    def test_command_error_preserves_stdout_and_stderr(self):
        completed = subprocess.CompletedProcess(
            ["npm", "publish"],
            1,
            stdout="npm error code E403\nforbidden",
            stderr="npm notice package contents",
        )
        message = npm_publish._command_error(completed, "fallback")
        self.assertIn("E403", message)
        self.assertIn("npm notice package contents", message)
