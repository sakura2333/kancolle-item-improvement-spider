...
def _download_useitem_image(consumable: ConsumeItem, png_url: str | None) -> None:
    ...
    Type 1 consumables are use-items and keep the historical `cache/images`
    location. Type 0 consumables are equipment images; they are stored in a
    separate cache namespace so public package consumers can distinguish
    equipment art from use-item icons.
    """
    ...
    if int(consumable.type) == 1:
        ...
        download_pic(url=png_url, save_path=f"cache/equipment-images/{consumable.id}.png")


def download_equipment_image(equipment_id: int, png_url: str | None) -> None:
    if not png_url or not equipment_id:
        return
    download_pic(url=png_url, save_path=f"cache/equipment-images/{int(equipment_id)}.png")
...
def _detail_equipment_image_url(page_node) -> str | None:
    values = page_node.xpath(
        ...
        "//img[not(starts-with(@src, 'data:'))]/@src"
        " | //div[contains(concat(' ', normalize-space(@class), ' '), ' detail-image ')]"
        "//div[contains(concat(' ', normalize-space(@class), ' '), ' weapon-imgs ')]"
        "//img[not(starts-with(@data-src, 'data:'))]/@data-src)[1]"
    )
