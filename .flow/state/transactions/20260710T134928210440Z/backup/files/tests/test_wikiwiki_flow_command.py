...
class WikiWikiFlowCommandTest(unittest.TestCase):
    def test_daily_flow_runs_catalog_then_crawl(self):
        with _make_root() as temp_name:
            ...
            self.assertIn("start2_command.py", " ".join(calls[0]))
            self.assertIn("catalog", calls[1])
            self.assertIn("--kind", calls[1])
            self.assertIn("all", calls[1])
            self.assertIn("crawl", calls[2])
            self.assertNotIn("--refresh", calls[1])
            self.assertNotIn("--daily-limit", calls[2])
    ...
    def test_full_flow_refreshes_catalog_and_uses_full_limit(self):
        with _make_root() as temp_name:
            ...
            self.assertNotIn("--refresh", calls[2])
            index = calls[2].index("--daily-limit")
            self.assertEqual(calls[2][index + 1], str(wikiwiki_command.FULL_REFRESH_LIMIT))


    def test_probe_flow_runs_three_item_chain_without_claiming_ready(self):
        with _make_root() as temp_name:
            root = Path(temp_name)
            calls: list[list[str]] = []
            ...
            with mock.patch.object(wikiwiki_command, "run_logged", side_effect=fake_run_logged):
                result = wikiwiki_command.run(root, ["probe"], {}, None)
            ...
            self.assertIn("crawl", calls[2])
            index = calls[2].index("--daily-limit")
            self.assertEqual(calls[2][index + 1], "3")
            self.assertIn("3 条数据 probe", result["current"])
            self.assertIn("./flow smoke", result["next"])

    def test_probe_rejects_full_flag(self):
        with _make_root() as temp_name:
            root = Path(temp_name)
            result = wikiwiki_command.run(root, ["probe", "--full"], {}, None)

            self.assertEqual(result["exitCode"], 2)
            self.assertIn("probe", result["current"])
