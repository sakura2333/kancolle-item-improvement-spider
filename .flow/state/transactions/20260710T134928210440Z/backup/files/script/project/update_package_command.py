...
def _package_version_from_name(value: object) -> int:
    import re
...
def _next_package_version(baseline: dict[str, Any]) -> int:
    value = baseline.get("packageVersion")
    if value not in (None, ""):
        try:
            return int(value) + 1
        except (TypeError, ValueError) as exc:
            raise UpdatePackageError(f"Flow baseline packageVersion 不是整数：{value}") from exc
    inferred = max(
        _package_version_from_name(baseline.get("lastAppliedPackage")),
        _package_version_from_name(baseline.get("source")),
    )
    return inferred + 1 if inferred else 1000


def _output_path(root: Path, args: list[str], project_id: str, package_version: int) -> Path:
    if "--output" in args:
    ...
    return (output_root / f"{project_id}-{package_version}.zip").resolve()


def _package_metadata(artifact_name: str, *, project_id: str, package_id: str, package_version: int, from_version: str, to_version: str, base_identity: dict, target_identity: dict, manifest: dict, sha256: str | None = None) -> dict[str, Any]:
    payload = {
        "schemaVersion": 3,
...
def run(root: Path, args: list[str], config: dict, loader=None) -> dict:
    ...
    from_package_version = _package_version_from_name(baseline.get("lastAppliedPackage")) or _package_version_from_name(baseline.get("source"))
    if baseline.get("packageVersion") not in (None, ""):
        try:
            from_package_version = int(baseline.get("packageVersion"))
        except (TypeError, ValueError) as exc:
            raise UpdatePackageError(f"Flow baseline packageVersion 不是整数：{baseline.get('packageVersion')}") from exc
    package_version = from_package_version + 1 if from_package_version else 1000
    ...
    output = _output_path(root, args, project_id, package_version)
    ...
    manifest = {
        "schemaVersion": 2,
        "packageType": "update",
        "packageId": f"{project_id}-{package_version}",
        "packageVersion": package_version,
        "fromPackageVersion": from_package_version or None,
        "projectId": project_id,
        ...
        "targetIdentity": target_identity,
        "from": {
            "packageVersion": from_package_version or None,
            "version": from_version,
            "projectVersion": from_version,
    ...
    package_id = f"{project_id}-{package_version}"
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
        archive.writestr("flow-package.json", json.dumps(metadata, ensure_ascii=False, indent=2) + "\n")
    ...
    return result(
        ...
        [
            ...
            "Flow metadata：内嵌 flow-package.json",
            f"变更文件：{len(changed)}，删除：{len(delete)}",
            f"ZIP SHA-256：{_sha256_file(output)}",
        ],
        [],
        "复制到下载目录后直接执行 ./flow",
        "单文件包不携带 .git；失败按 update.transaction 回滚",
    )
