...
TOOL_DIR = Path("automation") / "acquire" / "wikiwiki"
CRAWLER_SCRIPT = TOOL_DIR / "crawler.py"
FULL_REFRESH_LIMIT = 10000
START2_SLOTITEM_PATH = Path("dist") / "data-pipeline" / "start2_data" / "api_mst_slotitem.json"
...
def _usage_error(message: str) -> dict:
    return result(
        ...
        [],
        [message],
        "执行 ./flow smoke、./flow wikiwiki、./flow wikiwiki --full、./flow wikiwiki probe、./flow wikiwiki config 或 ./flow wikiwiki session",
        "无需回滚",
        2,
    )
...
def _base_command(root: Path, subcommand: str) -> list[str]:
    return [
        ...
        subcommand,
        "--start2",
        START2_SLOTITEM_PATH.as_posix(),
    ]


def _start2_command() -> list[str]:
    return ["{python}", "script/project/python_runner.py", "script/project/start2_command.py", "ensure"]
...
def _parse(args: list[str]) -> tuple[str, bool, bool, int | None, bool] | dict:
    values = list(args)
    action = "crawl"
    skip_start2 = False
    if values and values[0] in {"session", "config", "smoke", "probe"}:
        action = values[0]
        values = values[1:]
        if action == "smoke":
            action = "probe"
    ...
    if action == "session" and (full or refresh or daily_limit is not None or skip_start2):
        return _usage_error("session 只用于人工恢复浏览器会话，不接受 --full、--refresh、--daily-limit 或 --skip-start2")
    if action == "config" and (full or refresh or daily_limit is not None or skip_start2):
        return _usage_error("config 只用于生成/查看本地配置路径，不接受 --full、--refresh、--daily-limit 或 --skip-start2")
    if action == "probe" and full:
        return _usage_error("probe 是 WikiWiki 子系统小样本采集入口，不接受 --full；完整小样本链路请执行 ./flow smoke")
    if skip_start2 and action in {"session", "config"}:
        return _usage_error("session/config 不接受 --skip-start2")
    if action == "probe" and daily_limit is None:
        daily_limit = 3
    return action, full, refresh, daily_limit, skip_start2
...
def run(root: Path, args: list[str], config: dict, loader=None) -> dict:
    ...
    action, full, refresh, daily_limit, skip_start2 = parsed
    if action == "config":
    ...
    if not skip_start2:
        start2_log = run_logged(root, _start2_command(), "wikiwiki-start2")
    ...
    if receipt_path.is_file():
        ...
        completed.append("Flow run 感知：ready receipt 会被 ./flow status 展示，并被 ./flow run 复用")
    else:
        receipt = {}
        if full:
            return result(
    ...
    if receipt and not receipt.get("ready"):
        ...
        if action == "probe":
            return result(
                "成功",
                "WikiWiki 3 条数据 probe 已完成，但 source receipt 尚未 ready",
                completed,
                incomplete,
                "full source acquisition 请执行 ./flow wikiwiki --full；完整链路小样本请执行 ./flow smoke",
                "无需回滚；source cache 与断点保留在 .flow/local",
            )
        return result(
            "成功",
    ...
    if action == "probe":
        current = "WikiWiki 3 条数据 probe 完成"
        next_step = "full source acquisition 请执行 ./flow wikiwiki --full；完整链路小样本请执行 ./flow smoke"
    elif full:
