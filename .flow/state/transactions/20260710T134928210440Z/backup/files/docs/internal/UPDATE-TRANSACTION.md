...
保护 `.git`、`.flow/local.json`、`.flow/state`、`.venv`、`.idea`、日志、dist、`.flow/local/source-cache` 和全部 generated-state。更新与回滚自动提交 project-owned 内容，但不自动推送、打 Tag 或发布；受保护状态即使已暂存，也会在提交前自动退回未暂存状态。
...
- `projectVersion` 只记录业务版本变化，不参与筛包硬条件。自动更新从当前 `packageVersion=N` 只定位 canonical `N+1` 包；版本号负责低成本索引，包内 project identity 与 `from.contentHash == current contentHash` 负责最终验真。
- `1001+` update 包是单文件自描述 ZIP，机器读取包内 `flow-package.json` / `update-manifest.json`，不再依赖外部 sidecar。
- canonical 文件名固定为 `<projectId>-<packageVersion>.zip`。同一 project + 同一 packageVersion 在多个输入目录出现不同 `payloadHash` / `from.contentHash` / `to.contentHash` 时必须停止；相同制品副本可以去重，禁止按文件时间猜测。
- package sync 只修 `.flow/baseline.json`，不修改项目内容；只有当前 contentHash 能唯一匹配已知 update target 时才允许自动同步。
- `.flow/local.json.downloadRoot` 是唯一自动 update 输入目录；正常入口为 `./flow update`，无需传目录。已归档 applied 包只作审计，不再参与自动扫描。
- 每个 package 必须由独立 Flow 子进程应用。应用成功后重新从磁盘启动新版 Flow，重新读取 baseline 的 `packageVersion/contentHash`，再定位新的 `N+1`；禁止在同一 Python 模块图中连续应用跨版本更新。
