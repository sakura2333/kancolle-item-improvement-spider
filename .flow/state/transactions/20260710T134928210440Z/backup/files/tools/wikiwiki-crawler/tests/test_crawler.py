...
MIGRATE = ROOT / "tools" / "wikiwiki-crawler" / "migrate_existing_html.py"
sys.path.insert(0, str(ROOT))

from automation.acquire.wikiwiki.page_catalog import load_name_aliases, normalize_name, parse_card_catalog, resolve_name  # noqa: E402
from automation.acquire.wikiwiki import crawler  # noqa: E402
...
class WikiWikiPageCatalogTest(unittest.TestCase):
    ...
    def test_exact_catalog_match_overrides_pending_source_exclusion(self):
        catalog = {
            "schemaVersion": 1,
            "kind": "equipment",
            "entries": [{
                "wikiName": "SB2U-2",
                "normalizedName": "SB2U-2",
                "url": "https://wikiwiki.jp/kancolle/SB2U-2",
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            path.write_text(json.dumps({
                ...
                "exclusions": [{
                    ...
                    "status": "accepted",
                    "start2Name": "SB2U-2",
                    "reason": "Pending source page during Start2 rollout.",
                }],
            }), encoding="utf-8")
            result = resolve_name(
                "SB2U-2",
        ...
        self.assertEqual(result["status"], "resolved")
        self.assertEqual(result["matchType"], "exact-name")
        self.assertEqual(result["url"], "https://wikiwiki.jp/kancolle/SB2U-2")
...
class WikiWikiCrawlerToolTest(unittest.TestCase):
    ...
    def test_catalog_inspect_and_capture_with_fake_curl(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            (root / "dist/data-pipeline/start2_data").mkdir(parents=True)
            (root / "dist/data-pipeline/start2_data/api_mst_slotitem.json").write_text(
                json.dumps([
