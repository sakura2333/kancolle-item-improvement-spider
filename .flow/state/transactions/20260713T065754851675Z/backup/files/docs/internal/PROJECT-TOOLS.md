...
npm 双制品由公开 `automation/release/**` 管理：Data Build 在版本规划前读取 npm 最高正式版本 `RELEASES.json` 的消费者 `contentDigest`。相同 digest 固定复用 npm 已有版本，本次 Build 会自动补齐缺失的 canonical/`improvement2` dist-tag 与 `online`；只有 digest 不同才分配新 patch。发布决策不依赖上一次 Workflow 或 `online` 的完成状态；`release.yml` 仍可对任一冻结 Candidate 做补偿。Stable main 发布不构建、发布或对账 npm。
