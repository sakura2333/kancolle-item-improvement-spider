...
def _verify_tarball(
    ...
    *,
    ...
    with tarfile.open(tarball, "r:gz") as archive:
        ...
        content_digest = str((entry or {}).get("contentDigest") or "").strip().lower()
        if re.fullmatch(r"[0-9a-f]{64}", content_digest) is None:
            raise ProjectCommandError(
                f"npm tarball {result['version']} lacks a valid consumer contentDigest "
...
def build_release_set(
    ...
    *,
    ...
    if compatibility_digest != current_digest:
        raise ProjectCommandError(
            "current and improvement2 npm variants carry different consumer contentDigest values"
        )
...
def verify_release_set(path: Path) -> dict[str, Any]:
    ...
    if digests.get("current") != digests.get("improvement2"):
        raise ProjectCommandError(
            "npm release-set variants carry different consumer contentDigest values"
        )
    return payload
