...
def _validate_icons(directory: Path) -> set[int]:
    """Validate the legacy improvement2 useitem PNG projection."""
            ...
            with Image.open(asset) as image:
                if image.format != "WEBP" or image.size != expected_size:
                    ...
                    raise QualityGateError(
                        f"{label} must be {width}x{height} WebP: {asset}"
                    )
                image.verify()
        except QualityGateError:
            raise
        except Exception as error:
            raise QualityGateError(f"invalid {label} WebP asset: {asset}: {error}") from error
    return image_ids


def _validate_useitem_images(directory: Path) -> set[int]:
    return _validate_webp_images(
        directory,
        expected_size=(270, 270),
        label="use-item image",
    )
