...
def _useitem_source_cache_path(useitem_id: int) -> str:
    return f"cache/official/useitem/{int(useitem_id)}.png"
...
def _is_valid_webp(target: Path, *, expected_size: tuple[int, int]) -> bool:
...
def _encode_webp(
    ...
    *,
    ...
    if (
        ...
        and _is_valid_webp(target, expected_size=expected_size)
    ):
        return target
    target.parent.mkdir(parents=True, exist_ok=True)
    _validate_png(source, expected_size=expected_size)
    try:
        with Image.open(source) as image:
            image.load()
        ...
        raise RuntimeError(f"failed to encode {label} WebP: {source}: {error}") from error
    return target
...
def download_equipment_card(equipment_id: int) -> Path:
    ...
    return _encode_webp(
        source,
        CACHE_EQUIPMENT_IMAGE_DIR / f"{item_id}.webp",
        expected_size=EQUIPMENT_CARD_SIZE,
        label="equipment",
    )
...
def download_useitem_card(useitem_id: int) -> Path:
    ...
    source = _download_first(
        ...
        _useitem_source_cache_path(item_id),
    )
    _validate_png(source, expected_size=USEITEM_CARD_SIZE)
    _encode_webp(
        source,
        CACHE_USEITEM_WEBP_DIR / f"{item_id}.webp",
        expected_size=USEITEM_CARD_SIZE,
        label="useitem",
    )
    # Keep returning the official PNG source for compatibility with existing
    # acquisition callers. The canonical package consumes the generated WebP.
    return source
