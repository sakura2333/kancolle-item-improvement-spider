...
def build_assistant_day_reverse_index(
    ...
    *,
    ...
    for day_index, day_key, day_name in DAY_DEFINITIONS:
        ...
        for ship_id in sorted(by_day_ship[day_index]):
            ...
            if equipment_count != threshold:
                ...
                mismatch_row["expectedEquipmentCount"] = threshold
                mismatch_row["equipmentCountDelta"] = equipment_count - threshold
                equipment_count_mismatches.append(mismatch_row)
                if equipment_count < threshold:
                    under_expected.append(mismatch_row)
            if equipment_count > threshold:
                over_row = dict(row)
                over_row["expectedEquipmentCount"] = threshold
                over_row["equipmentCountDelta"] = equipment_count - threshold
                over_threshold.append(over_row)
        days.append({
            "dayIndex": day_index,
            "dayKey": day_key,
            "dayName": day_name,
    ...
    equipment_count_mismatches = sorted(
        equipment_count_mismatches,
        key=lambda item: (
            int(item["dayIndex"]),
            int(item["shipId"]),
    ...
    return {
        ...
        "threshold": threshold,
        "expectedEquipmentCount": threshold,
        ...
        "equipmentCountMismatchShipDayCount": len(equipment_count_mismatches),
        "equipmentCountMismatches": equipment_count_mismatches,
        "underExpectedShipDayCount": len(under_expected),
        "underExpected": under_expected,
...
def write_assistant_day_reverse_index(
    ...
    *,
    ...
    return {
        ...
        "expectedEquipmentCount": payload["expectedEquipmentCount"],
        "equipmentCountMismatchShipDayCount": payload["equipmentCountMismatchShipDayCount"],
        "underExpectedShipDayCount": payload["underExpectedShipDayCount"],
...
def _equipment_text(equipment: Mapping[str, Any]) -> str:
    ...
    if route_ids:
        route_text = " routes=" + ",".join(route_ids)
...
def _markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"expectedEquipmentCount: {payload.get('expectedEquipmentCount', payload.get('threshold'))} equipments per ship per day",
        f"recordCount: {payload.get('recordCount')}",
        f"routeCount: {payload.get('routeCount')}",
        f"shipDayCount: {payload.get('shipDayCount')}",
        f"equipmentCountMismatchShipDayCount: {payload.get('equipmentCountMismatchShipDayCount')}",
        f"underExpectedShipDayCount: {payload.get('underExpectedShipDayCount')}",
    ...
    under = payload.get("underExpected") if isinstance(payload.get("underExpected"), list) else []
    if not under:
        lines.append("No ship/day has fewer than the expected equipment count.")
    else:
        for row in under:
            ...
            lines.append(
                ...
                f"expected={row.get('expectedEquipmentCount', payload.get('expectedEquipmentCount'))} "
                f"delta={row.get('equipmentCountDelta')}"
            )
    ...
    lines.append("")
    lines.append("## Over expected")
    lines.append("")
    over = payload.get("overThreshold") if isinstance(payload.get("overThreshold"), list) else []
    if not over:
        lines.append("No ship/day has more than the expected equipment count.")
    else:
        for row in over:
            if not isinstance(row, Mapping):
                continue
            ...
            for equipment in equipments:
                if isinstance(equipment, Mapping):
                    lines.append(f"  - {_equipment_text(equipment)}")
    lines.append("")
    lines.append("## All equipment count mismatches")
    lines.append("")
    mismatches = payload.get("equipmentCountMismatches") if isinstance(payload.get("equipmentCountMismatches"), list) else []
    if not mismatches:
        lines.append("No named assistant ship/day deviates from the expected equipment count.")
    else:
