...
- Python 3.10 或更高版本；项目运行入口只依赖当前 `python3`
- Node.js 20+ 与 npm（校验或使用数据包时需要）
- uv 可选，仅用于按 `uv.lock` 创建和同步隔离环境

依赖版本必须与 `pyproject.toml` 一致。推荐使用 uv 按锁文件创建隔离环境，但 uv 不是项目运行入口：

```bash
uv sync --locked --python python3
source .venv/bin/activate
```

也可以使用其他 Python 依赖安装方式，只要当前 `python3` 中的直接依赖版本与项目声明一致。

Windows PowerShell 激活命令为 `.venv\Scripts\Activate.ps1`。无论 Python 由系统包、Homebrew、pyenv 或其他工具提供，项目只检查实际启动项目命令的解释器版本和依赖。

## 运行

```bash
python3 service/akashi_list/akashi_list_spider.py

# 与 GitHub Actions 相同的严格数据包构建入口
python3 -m service.data_package.cli build --strict
```
...
- `KANCOLLE_ASSET_BASE_URLS=https://server-a/,https://server-b/`：可选，覆盖官方图片资源服务器候选地址。
...
dist/data-pipeline/assets/equip/
...
Akashi 路线转换完成后，由独立 official-assets 阶段按最终引用 ID 获取官方 useitem/equip 原图并保存到 Source Cache；官方图片使用独立缓存命名空间，缓存有效期为 180 天；旧 Akashi 图片缓存不会被迁移或复用，并会在恢复旧 Source Bundle 后从下一份 Bundle 中清除。`latest` 把 useitem 与装备统一发布为 `{id}.webp`，`improvement2` 仅保留旧插件需要的 schema-3 改修数据和 `assets/useitem/{id}.png`。
执行严格数据构建后，终端结果和完整运行日志都会显示当前来源权重与置信度；机器摘要位于 `dist/data-pipeline/local-validation.json`。

## 数据来源

- Akashi List：改修路线、材料、星期、二号舰和更新目标；
- start2：舰船、装备、装备类型和消耗品的 ID/名称映射，以及装备图片资源版本；
- 舰 C 官方 `kcs2/resources`：useitem 原始 PNG 与装备 `slot/card` 原图；canonical 构建统一编码为 WebP quality 93，旧 `improvement2` 仅保留 useitem PNG；
...
公开自动化拆分为三个职责：`source-acquire.yml` 只采集并冻结 Source Bundle；`data-build.yml` 消费冻结来源，完成离线计算、验证和不可变 Candidate，并在 npm 实际业务内容变化时自动发布 `latest`、`improvement2` 和 `online`；`release.yml` 只用于从既有冻结 Candidate 补偿或对账发布。Data Build 会先把 canonical 与 `improvement2` 真实打成 tgz，再通过同一个 npm 业务身份方法与 Registry 实际 tarball 比较。该身份覆盖数据文件、入口代码、类型声明、Schema、校验脚本和规范化后排除 `version` 的 `package.json`；生成 manifest、README、CHANGELOG、RELEASES、许可证摘要和审计文件不参与升版判断。两个变体均相同则复用 npm 现有版本，缺失变体、dist-tag 或 `online` 只做同版本对账；任一已存在变体的业务内容发生变化则分配下一 patch。包内历史自报 digest 只作记录，不再参与决策。`online` 只是 npm 正式状态的可修复投影。
...
```bash
python3 -m service.data_package.equipment_acquisition_raw_parse
```
...
```bash
python3 -m automation.acquire.wikiwiki.crawler catalog --kind all
python3 -m automation.acquire.wikiwiki.crawler inspect
python3 -m automation.acquire.wikiwiki.crawler crawl
```
...
console.log(data.assets.equipmentPath(61)) // assets/equip/61.webp
```
