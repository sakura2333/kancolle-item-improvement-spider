...
- `./flow check` 会验证 Python 3.14、`uv.lock` 与锁定环境；环境未同步时只给出 `mise exec -- uv sync --locked`，不会把缺包误报为业务代码失败。
...
```bash
mise exec -- uv run --locked script/project/equipment_acquisition.py
```

它只读取 `.flow/local/source-cache/` 中已采集的 WikiWiki 原始页面，不进行网络请求，并输出结构化获取方式、未分类证据和问题清单。舰娘证据关联 Start2；任务目录来自 `kcwikizh/kcQuests`，顶层数字 key 是 canonical `questKey`，code 与名称仅用于匹配和诊断。接受后的 `questKey` 会进入统一装备来源投影；正式 `flow run` 会增量获取完整 `quests-scn.json`，随后仅使用本地 Raw Cache 重建 WikiWiki 获取证据。原始证据和问题清单仍保留在 `dist/data-pipeline/sources/wikiwiki-equipment-detail/`。
...
CI 由 `automation.acquire.cli` 调度它并冻结 Source Bundle；本地维护仍可使用 `./flow wikiwiki` 作为 dev 包装入口。配置位于 `configs/wikiwiki-crawler.local.json`，Cookie、浏览器 profile、断点和 receipt 只写 `.flow/local/**`，通过校验的 HTML 写入 `.flow/local/source-cache/**`。旧版 crawler 私有 HTML 的一次性迁移工具保留在 `tools/wikiwiki-crawler/migrate_existing_html.py`，不进入公开 main。
