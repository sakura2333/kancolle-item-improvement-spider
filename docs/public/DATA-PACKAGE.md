...
- 装备获得关系；
- 特殊装备加成；
- 舰 C 官方 useitem 卡生成的 270×270 WebP quality 93；
...
`latest` 发布 schema-4 完整包和全 WebP 图片；`improvement2` 只发布同一候选生成的 schema-3 改修数据与 useitem PNG。release-set 分别记录 `current` 与 `improvement2` 两个实际文件 digest，并在冻结后重新打开两个 tarball 复验；发布阶段不得重新生成任何 tarball。

## 图片资产契约

- `latest/assets/useitem/{id}.webp`：官方 `useitem/card`（或历史 `card_`）270×270 PNG 的确定性 WebP 投影。
- `latest/assets/equip/{id}.webp`：官方 `slot/card` 390×390 原图的确定性 WebP 投影。
- `improvement2/assets/useitem/{id}.png`：只为旧插件保留的官方 PNG；该变体不包含 `equipment/` 或 `assets/equip/`。
- 两类 WebP 均固定 quality 93、alpha quality 100、method 6。
