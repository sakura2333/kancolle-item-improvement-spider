...
## Image asset manifest schema 2

`manifest.datasets.equipmentImages` describes the packaged equipment-card projection:
...
`latest` generates `assets/equip/{id}.webp` from the official 390×390 `slot/card` PNG and `assets/useitem/{id}.webp` from the official 270×270 useitem PNG, both without resizing. Encoding is fixed at WebP quality 93, alpha quality 100 and method 6. The `improvement2` package retains only `assets/useitem/{id}.png` beside schema-3 improvement data and excludes all equipment datasets/images. IDs are validated against Start2; Akashi image URLs are not used.

## `poi-plugin-item-improvement2` compatibility distribution

The canonical improvement detail remains schema 4. During the public data-build workflow, the release automation generates a second npm candidate from the same frozen canonical data:
...
improvement2 candidate
  ...
  assets/useitem/{id}.png: present
  equipment/ and assets/equip/: absent
```

The schema-3 detail is an explicit whitelist VO projection. It writes only the original schema-3 fields and naturally ignores all future canonical additions. The schema-2 list is copied byte-for-byte. The compatibility package keeps the old improvement and useitem PNG paths, but deliberately omits unrelated equipment datasets and equipment images.

The internal generated projection is stored under `dist/packages/kancolle-data/compat/poi-plugin-item-improvement2/` only as release input and quality evidence. It is not exported by the `latest` tarball. Data-quality snapshots keep separate schema-4 and schema-3 data digests, while npm patch allocation uses one package-business identity method over the actual canonical and `improvement2` tgz files. The method canonically serializes JSON and NeDB, hashes binary files directly, includes data plus public runtime code, types, schemas, validation scripts and stable package metadata, and excludes version, generated manifest, README, changelog, release history, license summary and audit output. Both variants matching the Registry reuses the existing patch; a missing variant is reconciled at that patch; any business-content difference allocates the next patch. Existing Registry tarballs are downloaded into the immutable Candidate, and `release.yml` remains a manual recovery path for that frozen Candidate.

## Unified equipment sources schema v2

`dist/packages/kancolle-data/equipment/sources.nedb` contains one record for every player equipment in Start2. All three arrays are required even when empty, and `developmentAvailable` is always a non-null boolean:
...
- `developmentAvailable`: canonical KCWiki `_buildable` boolean, joined to Start2 only by an exact normalized Japanese-name match. WikiWiki independently parses the current equipment infobox `備考` row and cross-validates this value. A null/non-boolean flag or a source conflict emits a red `ERROR` and stops the projection.

The source-side projection under `dist/data-pipeline/sources/equipment-sources/` stores input hashes, added/changed/removed deltas, and `development-cross-validation.nedb`. It is generated state and is not part of code update identity.
