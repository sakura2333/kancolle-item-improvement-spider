...
def write_ready_lock(output: Path, manifest: dict) -> dict:
...
def verify_ready_lock(bundle: Path, manifest: dict) -> dict:
    ...
    if set(payload) != expected_keys:
        raise BundleError("source bundle ready lock fields are invalid")
    completed_at = payload.get("completedAt")
    if not isinstance(completed_at, str) or not completed_at:
        raise BundleError("source bundle ready lock completedAt is invalid")
    return payload
