...
_DEFAULT_CONFIRMATION_RETRIES = 25
_DEFAULT_CONFIRMATION_DELAY_SECONDS = 5.0
_AUDIT_MESSAGE_LIMIT = 6000
...
def _bounded_command_output(
    ...
    fallback: str,
    *,
    limit: int = _AUDIT_MESSAGE_LIMIT,
) -> str:
    """Keep both the beginning and the error-bearing tail of noisy npm output."""

    text = _command_error(completed, fallback)
    if len(text) <= limit:
        return text
    marker = "\n... npm output truncated; tail preserved ...\n"
    head = min(1200, max(0, limit // 4))
...
def load_package_result(path: Path) -> dict[str, Any]:
    ...
    tarball = Path(str(payload["tarball"])).expanduser()
...
def _poll_dist_tag(
    *,
    ...
    for attempt in range(attempts):
        try:
            last_tags = query_dist_tags(package, registry=registry, runner=runner)
            if last_tags.get(tag) == version:
                return last_tags, None, attempt + 1
            last_error = None
        except NpmPublishError as exc:
            last_error = str(exc)
        if attempt + 1 < attempts:
            time.sleep(max(0.0, retry_delay))
    return last_tags, last_error, attempts
...
def _ensure_dist_tag(
    *,
    ...
    completed = runner(command)
    after, poll_error, poll_attempts = _poll_dist_tag(
        package=package,
        version=version,
        tag=tag,
        registry=registry,
    ...
    if after.get(tag) == version:
        return {
            ...
            "confirmationAttempts": poll_attempts,
        }
    if completed.returncode != 0:
        raise NpmPublishError(
            _bounded_command_output(completed, "npm dist-tag add failed")
        )
    if poll_error:
        raise NpmPublishError(
            f"npm dist-tag {tag!r} confirmation failed: {poll_error}"
        )
    raise NpmPublishError(
        f"npm dist-tag {tag!r} was not confirmed at {version} "
        f"after {poll_attempts} attempts"
    )
...
def reconcile_npm_publish(
    *,
    ...
    retries: int = _DEFAULT_CONFIRMATION_RETRIES,
    retry_delay: float = _DEFAULT_CONFIRMATION_DELAY_SECONDS,
    runner: CommandRunner = _default_runner,
) -> dict[str, Any]:
    ...
    try:
        ...
        audit["publishAttempt"] = {
            ...
            "message": _bounded_command_output(completed, "") or None,
        }
        audit["registryConfirmationPolicy"] = {
            "retries": max(1, retries),
            "retryDelaySeconds": max(0.0, retry_delay),
            "maximumWaitSeconds": max(0, max(1, retries) - 1)
            * max(0.0, retry_delay),
            "preferOnline": True,
        }

        remote, poll_error, poll_attempts = _poll_release(
            package=package,
            version=version,
            registry=resolved_registry,
            retries=retries,
        ...
        audit["registryConfirmationAttempts"] = poll_attempts
        if remote is None:
            audit["status"] = (
            ...
            audit["error"] = _bounded_command_output(
                completed,
